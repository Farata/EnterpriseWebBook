...
<4> <vik>A `MessageEvent` is sent to clients when data is received from the server and delivered to the listener indicated by the WebSocket object's `onmessage` handler. `e.data` property of the message event contains the received message.</vik>
...
<vik>
When in your application you transmit JSON over the wire in what case the text opcode will be set for frames. When your code emits binary data it will be represented in a browser specific `Blob` object or `ArrayBuffer` object and send inside frame with the binary opcode.

NOTE: You must choose the type you want to use for incoming binary data on the client before reading the data using `webSocket.binaryType = "blob"` or `webSocket.binaryType = "arraybuffer"`.

For incoming data it's good idea  to investigate incoming data because opcodes not exposed to client side API and developer never know what type of data arrived.
...
The connection is terminated by sending frame with the close opcode `0x08`.

There the pattern to exchange close opcodes first and then let the server shut down. The client is supposed to give the server some time to close the connection before attempting to do that on its own. The close event can also signal why it terminated the connection.
