...
+
NOTE: In this simple scenario, the client's code doesn't send any explicit information to help the server to identify the client. As soon as the handshake completes successfully, the code that implements  `WebSocket` protocol provides the endpoint an active WebSocket session. The server has all required data about the client encapsulated in session object. This server-side object represents the conversation between two web socket endpoints.
+
...
Let's see whatever is described in the <<HANDSHAKE>> action with the help of Chrome Developer Tools, which provide the information about the initial handshake <<FIG8-4>>. This can be viewed in the Network tab after selecting the path of the `WebSocket` endpoint.
