...
In Chapter 8 you had a chance to see how a Web application can be sliced (see Aptana's project-15-dynamic-modules) into several modules using Require.js framework.  We'l take this project as a base and will create a new one: project16-websocket-auction adding to it the new modules supporting the auction. 
...
Once the user clicks the menu Way to Give the framework require.js has to load code of WebSocket-based auction module. Here is the code of `app.js` - the entry point of our Save Sick Child application.
...
require([], function() { // <1>
    (function() {
        ...
        var handleClick = function(event) {
                ...
                if(module !== null) {           
                    module.startAuction();      // <4>
                } else {
                    module = "loading";         // <5>
                    require(
                    ["modules/way-to-give"], function(WayToGive) { // <6>
                        module = new WayToGive();
                        module.renderControls();                   // <7>
                    });
                }
            }
        way_to_give.addEventListener('click', handleClick, false); // <8>
    })();
});
----------------------------------------------------------------------
