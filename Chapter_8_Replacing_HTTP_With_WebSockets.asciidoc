...
<1> <vik>Not all browsers support WebSockets natively yet.</vik> Check if the +WebSocket+ object is supported by the browser.
...
As we explained earlier in this chapter, WebSockets data can be fragmented into multiple frames. The first frame that transmits the data has a opcode on it that indicates what sort of data is being transmitted:

- Opcode `0x01` indicates UTF-8 encoded text data.
- Opcode `0x02` indicates the binary data.

Most people will transmit JSON over the wire in which case you want to chose the text opcode. When you emit binary data it will be represented in a browser specific `Blob` object.
...
The first consideration in cross-platform communication is the message format The most common formats what widely used in enterprises are CSV, XML and JSON. They are both easy to create and parse and widely supported via many frameworks for serialization in most major development platforms. As good as XML and JSON are, they are still quite heavy since they are essentially transmitted as strings across the wire. The can be a waste of memory and bandwidth for both the client and server, especially for internal communications. The alternative is binary, which has traditionally been difficult for cross-platform communication.

===== CSV =====

CSV stands for "comma separated values". Essentially, the delimiter can be any character, not only comma. It depends on parser design and implementation. Another popular type of delimiter is `|` - "pipe" . Lets describe pros and cons of CSV message format.
Pros:

- This format is very compact. The overhead for service symbols is minimal. This major advantage of CSV what can help to reduce bandwidth.
- Simple to create and parse. Using standard JavaScript `String.split` method CSV message can be turned into array of values.

Cons:

- Not suitable for storing complex data structures and hierarchies. In auction example application, we need transfer to client the auction item attributes for each the Auction. In this case we can't simple use general purpose `String.split` and have to design complex parser. The problem with customizing CSV format is that it involves overhead of maintaining more and more complex parsers.

===== XML =====

XML stands for "extensible markup language". XML was created to represent hierarchal data structures.

Pros:

- Long time XML advertised as human-readable format.
- The most browsers have build-in XML readers and parsers
- XML can be validated against XSD or DTD schema.
+
XML schema is very useful mechanism. XML schema defines structure, content and semantic of XML document. Because of human-readability feature, XML schema used by non-developers to agree about an interchange format between different enterprise systems written event in different languages and technologies.

Cons:

- XML is very verbose because.
- XML validation on the client is very complex task.

_TBD_

===== JSON =====

JSON stands for JavaScript Object Notation, and it's a way of representing structured data that has native implementations of encoding and decoding in all modern browsers. JSON has picked up acceptance on the web for data serialization.

_TBD_

===== Google Protocol Buffers =====

Google Protocol Buffers is a framework for data serialization. It is language-neutral, platform-neutral, extensible mechanism for serializing structured data. You define how you want your data to be structured once, then you can use special generated source code to easily write and read your structured data to and from a variety of data streams.
...
+
<vik>
NOTE: As soon as the handshake completes successfully, the server side code that implements `WebSocket` protocol exposes WebSocket session object.  The session object encapsulates conversation between WebSocket endpoint (server - side) and remote endpoint (browser). Check documentation for your server side framework for details about how it handles and exposes the remote endpoints in API.
</vik>
+
...
Let's see whatever is described in the <<HANDSHAKE>> action.  With the help of Chrome Developer Tools (<vik>You can open it by choosing +View > Developer > Developer Tools+) you can get the information about the initial handshake </vik> <<FIG8-4>>. This can be viewed in the +Network+ tab after selecting the path of the `WebSocket` endpoint.

[[FIG8-4]]
.Initial WebSockets handshake in Chrome DevTools
image::images/fig_08_04.png[]

You can also click on +WebSockets+ on the bottom-right to show only the `WebSocket` endpoints. Click on +Frames+ in the right panel to view the actual frames being exchanged between the client and server. <<FIG8-5>>. The white colored rows represent incoming data, and the green rows - outgoing.

[[FIG8-5]]
.Monitoring WebSocket frames in Chrome Developer Tools
image::images/fig_08_05.png[]

For more gore details you can navigate your Google Chrome to secter URL - `chrome://net-internals`
<vik>
[[FIG8-6]]
...
[[FIG8-7]
.Details of socket connection
image::images/fig_08_07.png[]

[[FIG8-7]]
.Dev tools display only payload size but net internals display actual size of frame (include service metadata)
