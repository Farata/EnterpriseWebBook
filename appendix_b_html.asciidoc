...
<3> Call `pushState()` to remember the customer ID, and add the suffix _/who_we_are_ to serve as a path to the server-side RESTful request.
...
<5> Call `pushState()` to remember the customer ID, and add the suffix _/what_we_do_ will serve as a path to the server-side RESTful request. The page title is empty; the History API currently ignores it.
