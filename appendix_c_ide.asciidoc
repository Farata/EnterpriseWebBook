...
The code samples used in this book are available on https://github.com/Farata/EnterpriseWebBook_sources[GitHub]--they are grouped by chapters. If a chapter has code samples, look for the directory with the respective name.((("IDEs (integraged development environments)", "benefits of")))  
