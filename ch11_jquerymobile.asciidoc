...
<7> jQuery Mobile includes icons that you can use by specifying their names in the `data-icon` attribute. (More details are provided in the upcoming sidebar <<MMMMMMM>>.) The icon position is controlled by the attribute `data-iconpos`. If you don't want to show text, use `data-iconpos="notext"`.
