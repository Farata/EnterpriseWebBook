...
Take a look at the styling of the `navbar`. Our original `<ul>` HTML element didn't include the class `ui-grid-a`. jQuery Mobile couldn't find the predefined layout for a six-button navigational bar and "decided" to deploy `ui-grid-a`, which is a two-column grid (see the previous section, <<MMMMMMMM>>). 
