...
<3> Since the Page 2 is already shown on the screen, tapping on the button "Page #2" in the navigation bar should not change the page, hence `href="#"`. The `class="ui-state-persist"` makes the framework to restore the active state each time when the existing in the DOM page is shown. The file page3.html will have a similar anchor for the button "Page #3". You got the idea.
...
Let's start implementing the navigation based using the techniques described earlier in the section "Persistent Toolbars".
...
The index.html is the home page of our application. All other pages are located in the _pages_ folder. The javascript code is in the folder _js_, and fonts, images and CSS are in the folder _assets_. The source code of the index.html is shown below.
...
// <5>      
      ...
      data-id="persistent-footer">
        <div data-role="navbar" class="ssc-navbar">
          <ul>
            <li>
              <a href="pages/about.html" data-iconshadow="false" data-icon="info-sign" 
...
<1> The metatags to request the full screen mode and black status bar on iOS devices
...
/* prevent using images for icons - overwrite default classes */
...
.ui-icon-trash, .ui-icon-mappin, .ui-icon-direction, .ui-icon-heart, .ui-icon-wrench, .ui-icon-play, 
.ui-icon-pause, .ui-icon-stop, .ui-icon-person, .ui-icon-music, .ui-icon-wifi, 
.ui-icon-phone, .ui-icon-power, .ui-icon-lightning, .ui-icon-drink, .ui-icon-android {
  background-image: none !important;
}
/* if we prevent using images, we should provide custom icon for buttons - 
override default class via icon-font search */
...
Wile testing this initial version of the Save Sick Child application, note that the content of the About and Share pages is implemented as in mockups shown on <<FIG12-11>> and <<FIG12-17>>. The source code of the share.html is shown below (the about.html looks similar).
...
<html>
 ...
 <body>
   ...
   <div data-role="page" data-add-back-btn="true" id="Share">
     <div class="ssc-grid-header" data-role="header" data-position="fixed" data-tap-toggle="false" 
     data-id="persistent-header">
...
In this chapter you've got familiar with a simple to use non-intrusive mobile framework. We'll use the singe-page template, and in <<FIG12-25>>  you can see the structure and initial content of of our project. 