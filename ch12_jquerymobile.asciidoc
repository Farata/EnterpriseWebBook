...
Let's start implementing navigation based using the techniques described earlier in the section "Persistent Toolbars". The source code of the index.html is shown below.
...
<6> jQuery Mobile includes a number of icons that you can use by specifying their names in the `data-icon` attribute (read the Note on icons below). The icon position is controlled by the attribute `data-iconpos`.
...
<2> Override the jQuery Mobile CSS class selectors with the icon fonts. Whenever you create custom icon, jQuery assumes that it'll have a class with the name starting with `.ui-icon-`. But in HTML attributes you'll be using it without this prefix, e.g. `data-icon="arrow-l"`.

<3> Create some custom styles for the Save Sick Chile application.

While testing this initial version of the Save Sick Child application, note that the content of the About and Share pages is implemented as in mockups shown on <<FIG12-11>> and <<FIG12-17>>. The source code of the share.html is shown below (the about.html looks similar). After all the above explanations you should understand this code without further comments.
