...
<2> This is an interception of the event that's triggered if the code on the server is updated. The user is warned that the new version of the application has been downloaded. You can see more on this in the comments to _app.js_ in the section <<MMMMMMMM>>.
