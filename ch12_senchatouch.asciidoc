...
The +Campaigns+ store is used to display the markers on the map, where charity campaigns are active. Tapping the marker will show the description of the selected campaign, as shown in Figure <<FIG13-20>> (we tapped the Chicago marker). <<ex_campaign-js_store>> presents the code of the store _Campaigns.js_ .
