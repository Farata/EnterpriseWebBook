...
To demonstrate how to turn a web application into a hybrid one, we'll take the code of the jQuery Mobile version of the Save The Child application from <<sencha_touch>>. Initially, we'll just turn it into a hybrid PhoneGap application as is, without adding any native API calls. After that, we'll add to it the ability to work with a camera by using the PhoneGap API and create two builds for iOS and Android platforms. In this exercise, we'll use PhoneGap 3.3.(((hybrid mobile applications, PhoneGap, application programming))) 
