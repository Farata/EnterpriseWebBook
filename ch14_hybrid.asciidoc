...
.Testing Application on iOS Devices
*******
...
6. In Xcode, open the menu Window | Organizer, go to Provisioning profiles window, and refresh it. You should see the newly created provisioning profiled marked with a green bullet. 
...
After clicking on the iOS button, it revealed the error "No key selected, you must provide the signing key first". The error message also provides a https://build.phonegap.com/docs/config-signing-ios[link to the document] describing how to fix this erre.
