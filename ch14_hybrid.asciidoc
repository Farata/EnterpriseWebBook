...
Advocates of the native applications argue that PhoneGap applications may run slower comparing to the native ones, because there is a Web View (a Web renderer) between the application code and the native API of the device. The older versions of mobile OS may have slower Web rendering engines, but for the majority of enterprise applications the difference may not by noticeable given that  developers use best practices recommended for HTML5 applications. 

The UI components of the HTML5 framework of your choice may not look native enough too. But the main  selling point is that PhoneGap (and Cordova) allows you to leverage existing HTML/JavaScript developers' skills for all major mobile platforms, and their bridge to native APIs is easy to learn. 
...
In any case you'll need to install phonegap unility  according to the instructions from http://docs.phonegap.com/en/3.0.0/guide_cli_index.md.html#The%20Command-line%20Interface[the command-line interface]. Start with installing Node.js, which will also install its package manager _npm_ used for installing cordova (and phonegap library). We're developing on MAC OS X, and here's the command that will install phonegap: 
...
After generating Hello World with the _phonegap create_ command, the CLI creates files and directories shown on Figure <<FIG14-3>>. 
 
[[FIG14-3]]
.CLI-generated project Hello World
image::images/fig_14_03.png[] 

NOTE: If you'll be using the command exactly as it shown in the documentation (i.e. 	_phonegap create HelloWorld com.example.hello "Hello World"_), keep in mind that for iOS you'll be creating a  certificate, which has to be valid for applications packages located under  _com.example_. For more details see the sidebar "Testing Application on iOS Devices" later in this chapter.
...
This HTML file includes the code to load the cordova.js library and the initialization code from index.js.Then it calls `app.initialize()`. But if you look at <<FIG14-3>> the file cordova.js is missing. The CLI will add it to the project during the next phase of code generation when you'll run the command _phonegap platform add_ to add specific mobile platforms to your project. Let's look at the code of the index.js.
