...
NOTE: The information on the PhoneGap Web site may not exactly match the latest PhoneGap  distribution. To see what platforms  are supported by the current version of PhoneGap, just open subfolder _lib_ - the code for each supported platform has a corresponding subdirectory. For example, Figure <<FIG14-2>> will give you an idea on what's currently supported. 

[[FIG14-2]]
.What comes with PhoneGap 2.9
image::images/fig_14_02.png[] 

It's time for the PhoneGap Hello World, and we'll build it for iOS. We are going to generate the initial project using Cordova command line interface (CLI) as described in  http://docs.phonegap.com/en/2.9.0/guide_cli_index.md.html#The%20Cordova%20Command-line%20Interface[documentation]. We use Mac OS for development with pre-installed https://developer.apple.com/xcode/[Xcode IDE].
After generating Hello World with Cordova CLI, it created files and directories shown on Figure <<FIG14-3>>. 
...
This HTML includes the code to load the cordova.js library and the initialization code from index.js. But if you look at <<FIG14-3>> the file cordova.js is missing. We'll ask Cordova CLI to get it for us shortly. For now, let's look at the code of the index.js.
...
NOTE: The first command will run fine, because we have Xcode installed. The second command will be giving you errors until you install the latest Android SDK.

After running these commands, the initially empty directory _platforms_ will get additional content for each platform. 
