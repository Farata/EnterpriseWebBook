...
http://cordova.apache.org/[Cordova] is a library (and a build tool) that serves as a bridge between JavaScript and native API. Cordova started from the code donated by Adobe to Apache Software Foundation. Cordova is an open source platform created for building mobile applications with HTML5, but packaged as native ones. PhoneGap is a brand own by Adobe. It includes a packager and a build tool based on Cordova. Lots of developers from around the world work on Cordova, but Adobe used this library to create their own workflow of compiling and distributing mobile applications.  The closest analogy when the same software is used in different products are Chrome and Safari browsers that use the same open source rendering engine called Webkit.  Figure <<FIG14-1>> illustrates the interaction of  PhoneGap, Cordova, and Web applications. 
...
PhoneGap applications might be a little slower comparing to the native ones, because there is a Web View (a Web brenderer) between the application code and the native API of the device. The older versions of mobile OS may have slower Web rendering engines, but for the majority of enterprise applications the difference may not by noticeable. Of course, developers are encouraged to use best practices recommended for any HTML-based applications. 
...
In this section you'll see the entire process of building the Cordova/PhoneGap applications. After the Cordova library is installed on the developer's computer, PhoneGap 3.0 offers two major workflows. Each of the following workflows allows you to build a mobile application, but the main difference is where you build the application either locally or remotely:

1. Install all required mobile SDKs and tools for the mobile platforms you want to develop for, generate the initial project using Cordova Command Line Interface (CLI), add the application code, build locally, and test the application using IDE, simulators and physical devices.  
...
In any case you'll need to install cordova according to the instructions from http://docs.phonegap.com/en/3.0.0/guide_cli_index.md.html#The%20Command-line%20Interface[the command-line interface]. Start with installing Node.js, which will also install its package manager _npm_ used for installing cordova (and phonegap library). We're developing on MAC OS X, and here's the command that will install cordova: 
...
The above command installs the JavaScript file _cordova_ in _/usr/local/bin_ and the cordova library and its supporting files in the _/usr/local/lib/node_modules/cordova_ - Figure <<FIG14-2>> shows the snapshot of some of the files and directories that come with cordova. We've highlighted the create.js script, which will be used for generating Hello World and Save Sick Child projects. 
...
The time has come for a PhoneGap version of the  Hello World application. We are going to generate the initial project using  Cordova command line interface (CLI) as described in the same http://docs.phonegap.com/en/3.0.0/guide_cli_index.md.html#The%20Command-line%20Interface[document] we used for installing cordova in the section titled "Create the App". We'll be running the _cordova_ script passing it the command _create_:
...
NOTE: If you'll be using the command exactly as it shown in the documentation (i.e. 	_cordova create HelloWorld com.example.hello "Hello World"_), keep in mind that when you'll be creating the certificate required for running it on the iOS device, it has to be valid for applications packages located under  com.example. For more details see the sidebar "Testing Application on iOS Devices" later in this chapter.
...
This HTML file includes the code to load the cordova.js library and the initialization code from index.js.Then it calls `app.initialize()`. But if you look at <<FIG14-3>> the file cordova.js is missing. Cordova CLI will add it to the project during the next phase of code generation when you'll run the command _cordova platform add_ to add specific mobile platforms to your project. Let's look at the code of the index.js.
