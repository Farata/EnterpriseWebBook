...
The variables `address` and `isMarried` are visible only inside the function `addPersonalInfo()`. The variable `age` becomes global because of omission of the keyword `var`. In Chapter 3, you'll see another example of how to limit the scope of the variables to avoid polluting the global name space.
...
In the above code sample the JavaScript engine will not execute the function +calcTax()+ right after it's declared - the function has to be explicitly invoked . 
...
Technically, there can be other APIs that create objects, e.g. JSON.parse(), but let's keep things simple.
...
The easiest way to create a JavaScript object is by using the object literal notation. The code sample below starts with a creation of an empty object. The second line creates an object with one property +salary+ and assigns the value of 50000 to it. Finally, the instance of one more object is created and the variable +person+ points at it.  
...
Please not the difference in the content of the variables `__proto__` of the instances represented by `emp` and `emp2`. These two employees are inherited from two __different__ objects `Person`. Isn't it scary? Not really.
