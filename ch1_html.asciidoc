...
WARNING: IndexedDB doesn't offer a secure way of storing data. Anyone who has access to the user's computer can get a hold of the data stored in IndexedDB. Do not store any sensitive data locally.
...
To start any manipulations with the database you need to open a transaction in one of these modes. The `readonly` transaction allows multiple scripts to read from the database concurrently. This statement may raise a question - why would the user need a concurrent access to his local database is he's the only user of the application?  The reason being that the same application can be opened in more than one tab, or spawning more than one worker thread that need to access the local database. The `readonly` is the least restrictive mode.

If the application needs to modify or add objects to the database, open transaction in the `readwrite` mode. And if the database upgrade has to be done, use the `versionchange` mode. 

When a transaction is created, you should assign listeners to its `complete`, `error`, and `abort` events. If the `error` event is dispatched, the entire transaction is rolled back. Calling the method `abort()` will fire the `abort` event and will roll back transaction too.
...
request.onsuccess = function(event) {       
  ...
  var visitsData = [{
   ...
   var transaction = myDB.transaction(["visits"], "readwrite");
   var visitsStore = transaction.objectStore("visits");

  for (var i in visitsData) {

    var addRequest = visitsStore.add(visitsData[i]);

        addRequest.onsuccess = function(event){
          console.log("Added visit " + event.result);
        }

        addRequest.onerror = function(event){
          console.log("Adding visit failed" + event.result);
        }
  }
----
...
To delete and object specify its id:
