...
This chapter differs from all others because it includes less code, and is more of an overview of selected APIs that are included in HTML5 specification, namely Web Storage, offline Web applications, Web Workers, and History API. 

NOTE: To understand code samples included in this chapter you have to be familiar with JavaScript and some monitoring tools like Chrome Developers Tool. We assume that you are familiar with the materials covered in Chapter 1.
...
Another important feature of IndexedDB is that it accesses data asynchronously, so there won't be UI freezes while accessing large objects on disk. You make a request to the database and define the event handlers that should process errors or result when ready. Use Web Workers for long running data access, and since it asynchronous, the users will have a feeling that the application is pretty responsive, which is not the case if you'll be saving several megabytes of data with Web Storage API. Similarly to Web Storage, access to the IndexedDB databases is regulated by the same origin policy. 
...
IndexedDB doesn't require you to describe a formal structure of your stored object - any JavaScript object can be stored there. Think of a database as an _object store_ that allows to store similar objects, e.g. representing salesman's visits. Not having a formal definition of a database scheme is an advantage comparing to the relational databases where you can't store the data until the structure of the tables is defined.
...
Similarly to relational databases you create indexes based on the searches that you run often. For example, if you need to search on the contact name in the Visits database, create an index on the property `contactPerson` of the `Visit` object. But if in relational databases creation of indexes is done for performance reasons, with IndexedDB you can't run a query unless the index on the relevant property exists. The following code sample shows how to connect to the existing or create a new database `Visits`.   

[source, javascript]
----
var indexedDB = window.indexedDB;
var request = indexedDB.open('Visits');     // <1>

request.onsuccess = function(event) {       // <2>
    var myVisitsDB = request.result;

};

request.onerror = function (event) {        // <3>
    console.log("Can't retrieve visits info: " + event.target.errorCode);
};
...
<1> The browser invokes the method `open()` asynchronously requesting to establish the connection with the database. It doesn't wait for the  completion of this request, and the user can continue working with the Web page without any delays or interruptions. 

<2> When the connection is successfully obtained, the `onsuccess` function handler will be invoked, otherwise the control goes to the `onerror` handler. 

<3> Error handling is done here. The event object given to the `onerror` handler will contain the information about the error.

<4> The `onupgradeneeded` handler is the place to create a new storage or process the event when the database has to be modified.  

===== Creating a new Database and Versioning

In the world of traditional DBMS servers, when the database structure has to be modified, the DBA will do this upgrade, the server will be restarted and the users start working with the _new version_ of the database. With IndexedDB it works differently. Each database has a version assigned to it, and if the `onupgradeneeded` event is triggered, this means that the local user's version is older that needed for the freshly loaded application. 

The initial creation of the database has to be controlled in the  `onupgradeneeded` handler - the absence of the database also falls under the "upgrade is needed" case.  

The version of the IndexDB may need to be upgraded, and the method `open()` takes a second parameter - the database version to be used.The object returned by the method `open()` has a property `onupgradeneeded`, and you can assign an event handler to it in which you should create a database with `createObjectStore()` method. 

WARNING: IndexedDB doesn't offer a secure way of storing data. Anyone who has access to the user's computer can get a hold of the data stored in IndexedDB. Do not store locally any sensitive data. 
