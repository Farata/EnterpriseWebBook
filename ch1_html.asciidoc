...
At the time of this writing (Summer of 2013) local and session storage are supported by all modern Web browsers. Web SQL Database is not supported by Firefox and Internet Explorer. IndexedDB is not supported by Safari. To get the current status visit caniuse.com and search for the API you're interested in.

IMPORTANT: As opposed to cookies, the Web browser doesn't send the data saved in a local storage back to the server. The saved data is used only on the user's device.
...
The following code sample illustrates how to store and retrieve the custom Visit object. 
