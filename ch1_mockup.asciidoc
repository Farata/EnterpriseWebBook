...
Now let's examine the JavaScript code located in _main.js_. This code self-invokes the anonymous function, which creates an object-encapsulated namespace +ssc+ (short for Save The Child). This avoids polluting the global namespace. If we wanted to expose anything from this closure to the global namespace, we could have done this via the variable `ssc`, as described in <<appendix_a, Appenix A>> in the section "Closures."  
