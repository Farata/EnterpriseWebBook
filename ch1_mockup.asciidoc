...
But we web developers need to worry about other things, like making web pages responsive and light-weight. The first thing you need to do after receiving the prototype from Jerry is run it through Google Developer Tools or Firebug (see the section "Debugging JavaScript" in <<appendix_a>> for details) and measure the total size of the resources being downloaded from the server. If your application loads 1 MB or more worth of images, ask Jerry to review the images and minimize their size.
