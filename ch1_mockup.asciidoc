...
Two `select` drop-downs in the preceding code contain hard-coded values of all states and countries. For brevity, we've listed just a couple of entries in each. In <<using_ajax_and_json>>, we'll populate these drop-downs by using external data in JSON format. 
