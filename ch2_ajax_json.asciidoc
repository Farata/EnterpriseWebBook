...
The data to be used for drawing a pie chart in our canvas is stored in the file _data/chartdata.json_, but in the real world, the server-side code can generate it based on the up-to-the-second donation data and send it to the client. For example, you could do it as was explained in the section Json in Java above. This is the content of our file chartdata.json:
