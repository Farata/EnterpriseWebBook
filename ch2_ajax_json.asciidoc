...
Earlier in this chapter, you saw an example of populating states and countries in the donate form from HTML files.((("JSON (JavaScript Object Notation)", "states/countries dropdown menus"))) Now you'll see how to retrieve JSON data by making an AJAX call. In the web browser, open _project-04-2-donation-ajax-json_, which reads the countries and states from the files _countries.json_ and _us_states.json_, respectively. The beginning of the file _countries.json_ is shown here:
