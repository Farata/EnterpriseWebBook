...
Some time ago, one of the authors of this book was helping Mercedes Benz USA develop its consumer-facing web application, with which users could search for, review, and configure their next car. <<FIG4-5>> shows a snapshot taken from mbusa.com. Three rectangular areas at the bottom were created by web designers to display today's deals and promotions. The up-to-date content for these areas (in JSON format) was retrieved from a CMS server when the user visited mbusa.com. 
