...
This chapter differs from all others because it includes a little less code, and is more of an overview of selected APIs that are included in HTML5 specification, namely Web Storage, Application Cache, IndexedDB, localStorage, Web Workers, and History API. 
...
You'll see a more advanced example of the inter-component communication techniques that uses  _Mediator Design Pattern_ in <<mediator_section,corresponding section>> of Chapter 7 of this book.

.HTML5 Forms
*********
While this chapter is about selected HTML APIs, we've should briefly bring your attention to  improvements in the HTML5 `<form>` tag too.
...
NOTE: There is another option worth mentioning - http://www.w3.org/TR/webdatabase/[Web SQL Database]. The specification was based on the open source SQLite database, which probably was the wrong decision in the first place.  But the work on this specification is stopped and future versions of the browsers may not support it. That's why we are not going to discuss Web SQL Database in this book. 
...
Currently we use JavaScript API to work with Web Storage, but in the future any other programming language that can run in the browser will be able to work with the storage.  
...
*****
HTML5 specification includes an important feature - you can add to any HTML tag any number of http://bit.ly/9Udecy[custom non-visible attributes] as long as they start with `data-` and have at least one character after the hyphen. For example, this is absolutely legal in HTML5:
...
Behind the scenes, a custom framework can find all elements that have the `data-phone` attribute and generate some additional code for processing of the provided phone number. If this example doesn't impress you, wait till Chapter 12, where you'll learn how to use jQuery Mobile. The creators of this library use these `data-` attributes in a very smart way.
*****

=== Summary

In this chapter you've got introduced to a number of useful HTML5 APIs. You know how to check if a particular API is supported by your Web browser. But what if you are one of many enterprise developers that must use Internet Explorer of the versions earlier than 10.0? Google used to offer a nice solution:  http://www.google.com/chromeframe[Google Chrome Frame], which was a plugin for Internet Explorer.

The users had to install Ghrome Frame on their machines, and Web developers just needed to add the following line to their Web pages:
...
In this chapter we've briefly reviewed a number of useful HTML5 APIs, but we're not done yet. In Chapter 3 you'll learn how to use HTML5 GeoLocation API - our sample Save The Child application will include Google maps. Also,  Chapter 9 is completely devoted to working with WebSocket API - you'll see how to add an auction to you Web application.
