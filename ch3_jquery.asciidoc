...
There are libraries like http://jqueryui.com/[jQuery Core] that substantially minimizes the amount of manual coding while programming core functionality of a Web application. The jQuery UI library offers widgets, animations, advanced effects. The http://requirejs.org/[RequireJS] library is a module loader that allows to modularize HTML5 applications. There are also hundreds of micro libraries that can do just one thing and can be used ala cart (visit http://microjs.com[microjs.com] for details).
