...
Our Web designer, let's call him Jerry, is ready to start working on the _mockup_ (a.k.a. _wireframes_) - a set of images depicting various views of our Save Sick Child application. We expect him to deliver images with comments that would briefly explain what should change in a view if a user will make a certain action, e.g. clicks on the button. You can also think of a UI of an application as a set of states, and the user’s action result in your application’s transition from one state to the other. As nerds and mathematicians say
...
The goal of this section is to demonstrate the use of the HTML5 Geolocation API by adding a very practical feature - finding sick children based on their location. This way the users of the Save Sick Child can find the needy children in a particular geographical area. 
