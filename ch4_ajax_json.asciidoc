...
AJAX applications are subject to _the same-origin policy_ (the same protocol, port, and host) allowing `XMLHttpRequest` make HTTP requests only to the domains where the Web application was loaded from. 
...
But what if you need to dynamically make such requests either periodically (e.g. get all twits with a hashtag `#savisickchild` by sending an HTTP GET to request to `http://search.twitter.com/search.json?q=savesickchild&rpp=5&include_entities=true&with_twitter_user_id=true&result_type=mixed`) or when the user selected certain option on your Web page? You can achieve this by dynamically adding a `<script>` tag to the DOM object from your JavaScript code. Whenever the browser sees the new `<script>` element it executes it. Such script injection can be done like this:
...
Of course, this technique presents a danger if there is a chance that the JavaScript code sent by xyz.com may be intercepted and replaced by a malicious code. But it's not more dangerous that receiving any JavaScript from non-trusted  server. Besides, your handler function could always make sure that the received data is a valid object with expected properties, and only after that handle the data.
