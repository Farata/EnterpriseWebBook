...
Anyone who knows JavaScript understands that this is an object that represents a person, which has a nested object that represents an address. Note the difference with JavaScript literals: the names of the properties are always strings and must be taken into quotes. Representing the same object in XML would need a lot more characters (e.g. `<fname>Alex</fname>` etc). There are some other important differences between JSON and XML. The structure of the XML document can be defined using DTD or XML Schema, which simplifies the data validation. On the other hand, JSON data have data types, for example the age attribute in the above example is a `Number`. JSON also supports arrays while XML doesn't. 
...
The beginning of the file countries.json is shown below:
...
To make sure that your JSON data is properly formatted validate it using http://jsonlint.com/[JSONLint]. If you paste an ugly one-line JSON data JSLint will reformat it into a readable form. JSONLint can also compress JSON if you provide its URL with the suffix `?reformat=compress`.

==== Processing JSON streams in Java

In the Web browser we wanted to construct a JavaScript object from a JSON stream or turn the JavaScriot object into a JSON string. A similar task has to be performed on the server side. Our Save Sick Child application uses Java on the server side. There is a number of third-party Java libraries that can consume and generate JSON content. Besides the Java EE 7 specification made the first steps in prescribing how this process has to be standardized in any Java Application server.

=== Loading Charity Events using AJAX and JSON

The last example in Chapter 3 was about displaying various charity events on the Google map using multiple markers. But the data about these events were hard-coded in HTML file. After learning about AJAX and JSON it should not be too difficult to create a separate file with the information about charities in JSON format and load them using `XMLHTTPRequest` object.
