...
Get the latest version of Apache Tomcat from the Download section at http://tomcat.apache.org[Apache website]. At the time of this writing, Tomcat 7 is the latest production-quality build, so download the zip file with the Tomcat's Binary Distributions (Core). Unzip the file in the directory of your choice.
