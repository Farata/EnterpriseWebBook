...
Ext Core::
    A free-to-use http://bit.ly/1nWMdTy[JavaScript library] for enhancing websites. It supports DOM manipulation with CSS selectors, events, and Ajax requests. It also offers syntax to define and create classes that can extend from one another. The functionality of Ext Core is comparable to Core jQuery.
...
Download http://bit.ly/senchacmd-install[Sencha CMD]. Run the installer, and when it's complete, open the terminal or command window and enter the command *sencha*. You should see a prompt with all possible commands and options that CMD understands. 
...
TIP: Don't forget about the online tool JSFiddle, which allows you to test and share JavaScript code snippets. JSFiddle knows about Ext JS 4.2 already. For example, you can run the preceding code snippet by following this http://bit.ly/1ohWQDC[JSFiddle link]. If it doesn't render the styles properly, check the URL of _ext-all.css_ in the section External Resources.(((JSFiddle)))
...
TIP: The instance of any object has access to its class via the special variable http://bit.ly/1x56cX4[`self`]. 
...
<2> Unless you need to load some raw data from a third-party server provider, wrap your reader into a http://bit.ly/1qK2n6K[Proxy object]. Server proxies are used for implementing create, read, update, and destroy (CRUD) operations and include the corresponding methods `create()`, `read()`, `update()`, and `destroy()`. 
...
TIP: In <<appendix_b>>, we discuss the HTML5 local storage API. Ext JS has a class http://bit.ly/1qK2wHv[Ext.data.proxy.LocalStorage] that saves the model data locally if the web browser supports it.
...
The preceding `SSC.controller.Donate` doesn't use the config property `views`, but if it did, Ext JS would generate getters and setters for every view (the same is true for stores and models). It uses `refs` instead to reference components, and getters and setters will be generated for each component listed in `refs`; for example, `getDonatePanel()`. Lookup of such components is done based on the value in `selector` using the syntax compatible with http://bit.ly/1jCMwSw[`ComponentQuery`]. The difference between `refs` and the config property `views` is that the former generates references to instances of specific components  
...
We'll use the Eclipse IDE for Java EE Developers version, which is available free of charge at the http://bit.ly/V2y9Rx[Eclipse Downloads site]. The installation comes down to unzipping the downloaded archive. Then, double-click the Eclipse executable to start this IDE.  
...
image::images/ewdv_0406.png[float="true"]
...
<3> Configuring and creating a http://bit.ly/1rPJP52[pie chart]. The width of each sector is controlled by the `angleField` property, which is mapped to the field `donors` defined in the store `SSC.store.Donors` (see the code listing that follows).  
