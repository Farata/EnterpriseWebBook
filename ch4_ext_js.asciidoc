...
<3> For the mock-up mode, we use a JavaScript Object Notation (JSON)&#8211;formated file that contains an array of object literals (each object represents one donor). The _donors.json_ should look like the content of the `data` attribute in the code of `SSC.store.Donors` above. 
...
----
.Example 4-19 The Donors store with `ajax` proxy
...
----
.Example 4-20 The Donate controller
