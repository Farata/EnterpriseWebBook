...
The _app.js_ file is pretty short--it just declares SSC as the application name, views, and controllers. By adding the property `autoCreateViewport: true`, we requested the application to automatically load the main window, which must be called _Viewport.js_ and be located in the view directory:
