...
Up till now we've been using HTML, JavaScript and CSS to create the Save Sick Child Web application.  In the real world, developers try to be more productive by using JavaScript libraries and MVC frameworks. Libraries do not dictate how to structure the code of your application - they offer you components that allow to write less code. MVC frameworks force you to organize your code in layers implementing the Model-View-Controller design pattern.  There are more than a dozen MVC JavaScript frameworks that are being used by professional developers: http://backbonejs.org/[Backbone.js], http://www.sencha.com/products/extjs[ExtJS], AngularJS, http://emberjs.com/[Ember.js], http://knockoutjs.com/[Knockout] just to name a few. In this chapter we'll introduce the JavaScript library called jQuery. Chapter 6 is about the JavaScript framework ExtJS. Their mobile counterparts are called jQuery Mobile and Sencha Touch (see Chapters 12 and 13). 
...
TIP: While it's easier to write `$()` than `jQuery()`, keep in mind that if you decide to use in your application another library in addition to jQuery, with the chances are higher to run into a conflict of having another `$` than another `jQuery` in the global name space. Be careful of your find yourself in the "Another day, another $" position. 
...
----
TIP: If jQuery returns a set of elements that match the selector's expression, you can access its using array notation: `var theSecondDiv = $('div')[1]`.
...
loadData('data/us-states-list.json',                           // <4>
                         'usstateslist', $('#state'));   
loadData('data/counries-list.json',                            // <5>
                         'countrieslist', $('#counriesList'));
----

<1> The handler function to process the successfully retrieved data 

<2> Get the content of the HTML `<select>` element to populate with states or countries. The jQuery method `html()` uses the browser's `innerHTML` property.

<3> The handler function to process errors, if any

<4> Calling `loadData()` to retrieve states and populate the `#state` combobox. The `usstatelist` is the name of the root element in the json file us-states-list.json.

<5> Calling `loadData()` to retrieve countries and populate the `#countriesList` combobox
...
TODO: add the code sample from SSC

=== Plugins 

There are thousands of third-party jQuery plugins available in the http://plugins.jquery.com/[jQuery Plugin Registry]. Below are some of the useful plugins:


* http://www.jtable.org/[jTable] - AJAX-based tables (grids) for CRUD applications. This is a must have if you use jQuery for enterprise Web development
...
* http://www.oesmith.co.uk/morris.js/[morris.js] - a pluging for charting
...
<<FIG5-6>> shows the above code in action. After entering a one-character name and missing an email the user will see the corresponding error messages. These messages won't be shown until the user submits the form. But as soon as the user will fix any of them (e.g. enter one more letter in the name) the form will be immediately re-validated and the error messages will be removed as soon as the user fix the error.

[[FIG5-6]]
.Validator's Error Messages
image::images/fig_05_06.png[]

==== TODO Adding Image Slider
