...
We start this chapter with introduction of Node.js (or simply Node) - server-side JavaScript framework and Node Packages Manages (NPM). Node and NPM are a foundation of the tools that we're covering in this chapter.
...
* Bower is a package manager for the web that helps in maintaining application dependencies. 

* Yeoman is a collection of tools and best practices. It's a workflow. 
...
To use this book code samples you need to download and install Node from http://nodejs.org/download/[their web site]. Then follow http://gruntjs.com/getting-started[ the instructions from Grunt's  website] to install it on your machine.
