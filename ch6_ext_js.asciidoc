...
The first argument you specify to `define()` is a fully qualified class name, the second argument is an object literal that contains the class definition. For example, the next class `Header` has 200 pixel height, uses the `hbox` layout, has a custom config property `logo`, extends `Ext.panel.Panel`, which has to be rendered to some HTML element with id=content:
