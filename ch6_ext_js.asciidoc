...
In the previous chapter you've got familiar with the JavaScript library. Now we'll introduce you to a more complex product - a JavaScript framework http://www.sencha.com/products/extjs[Ext JS] from Sencha. This is one of the One of the most feature-complete frameworks available on the market, and you should give it a very serious consideration while deciding on the tooling for your next enterprise HTML5 application. 

The learning curve of this framework is steeper than with jQuery though, and this chapter is split into three sections. First, you'll go through a detailed Ext JS primer, then we'll develop a new version of the Save Sick Child application with Ext JS, and finally, you'll see how to improve the productivity of development with Ext JS and Java using the open source code generator called Clear Data Builder (CDB). 
