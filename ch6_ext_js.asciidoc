...
<1> Listing stores in your controller. Actually, in most cases you'd list stores is the `Ext.application` singleton as we did earlier. But if you need to, say dynamically create controllers, you don't have a choice but declare stores in such controllers.
...
`Ext.StoreManager` provides a convenience method to look up the store by store ID. If stores were automatically injected into `Ext.StoreManager` by the comntroller, the default store ID is it's short name, e.g. `Donors` for `SSC.store.Donors`:

[source, javascript]
----
var donorsStore = Ext.data.StoreManager.lookup('Donors');

// An alternative syntax to use StoreManager lookup 
var donorsStore = Ext.getStore('Donors'); 
----
