...
<2> The `control()` method of the controller take as arguments selectors to find the components with the corresponding event listeners to be added. For example, `'button[action=showform]'` means find a button that has a property `action` with the value `showform` - it has the same meaning as in CSS selectors.
...
The top level window is a `Viewport`, which will contain the `Header` and the `Donate` form views. 
...
<1> We assigned `appheader` as the `xtype` value of this view, which will be used as a reference inside the `Viewport`. 
...
Let's review the DonateForm next. It's a subclass of the `Ext.form.Panel` and contains the form with radio buttons, fields and labels. This component named `donateform` will be placed under the `Header` inside  the `Viewport`. 
