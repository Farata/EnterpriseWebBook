...
IMPORTANT: If your organization is developing Web applications with Ext JS without using Sencha CMD - it's a mistake. Sencha CMD is a very useful code generator and optimizer that also enforces the MVC principles of application design.
...
In Chapter 2 we stated that JavaScript doesn't have classes and that constructor functions were the closest to classes language elements. Ext JS extends the JavaScript language and introduces classes and a special way to define and instantiate them with functions `Ext.define()` and `Ext.create()`. Ext JS also allows to extend one class from another using the property `extend` and define class constructors using the property `constructor`. 
...
The next class `Header` has 200 pixel height, uses the `hbox` layout, has a custom config property `logo`, extends `Ext.panel.Panel`, which has to be rendered to some HTML element with `id=content`:
...
The above code sample also defines a constructor function, which calls the constructor of its superclass (in our case it's `Ext.panel.Panel`). 
It's a good practice to pass the standard JavaScript array `arguments` in one shot rather than trying to explicitly define each and every parameter.
...
The values of whatever custom properties you specify in the `config{}` section  of the class can be reassigned during the class instantiation. For example, the next code snippet will print sony.png for the first instance of the header, and sony_small.png for the second one. Please not that Ext JS automatically generated getters and setters for all config properties, which allowed us to use the method `getLogo()`.
...
TIP: Don't forget about an excellent little tool JSFiddle that allows you to test and share JavaScript code snippets, and JSFiddle knows about Ext JS 4.2 already. For example you can run the code snippet above by following this http://jsfiddle.net/yfain/yNva6/2/[JSFiddle link]. If it doesn't render properly, check the url of the ext-all.css in the section External Resources.

If a class you define dependencies on other classes, which must be  preloaded, use the `requires` parameter. For example, the next code snippet shows that the class `SSC.view.Viewport` requires the `Panel` and the `Column` classes. So the Ext JS loader will check if `Panel` and/or `Column` was not loaded yet, it'll load them automatically.  
...
`Ext.create()` is a preferred way of instantiation as it does more than the `new` operator that is also allowed in Ext JS. But `Ext.create()` can perform additional functionality, like lazy loading if need be (if you haven't used the option `require`). But there is even a shorter way of doing `define()` and `create()` in one step using `xtype`.

It seems that creators of Ext JS tried to model their class system after Java. For each class there is one instance of special class `Ext.Class`. Ext JS has familiar to Java developers concept of packages and fully qualified class names. On the other hand, Ext JS supports multiple inheritance, which Java doesn't. A class constructor can get any object as an argument, and Ext JS will use its property values to initialize the corresponding properties defined in the class, if they exist, and the rest of the properties will be created on the fly. 
...
TIP: In Chapter 1 we were discussing HTML5 local storage aAPI. Ext JS has a class http://docs.sencha.com/extjs/4.2.0/#!/api/Ext.data.proxy.LocalStorage[Ext.data.proxy.LocalStorage] that saves the model data locally if the Web browser supports it.
