...
<4> The +uglify+ task minifies files using http://lisperator.net/uglifyjs/[UglifyJS] - a compressor/minifier tool. Input and output of this task configured with `src` and `dest` properties of . UglifyJS produces significantly smaller version of the original script by reducing the http://en.wikipedia.org/wiki/Abstract_syntax_tree[Abstract Syntax Tree (AST)] and changing local variable names to single-letters.
