...
In JavaScript, the Module pattern is used to emulate the concept of classes. We're able to include both public/private methods and variables inside a single object, thus shielding particular parts from the global scope. What this results in is a reduction in the likelihood of our function names conflicting with other functions defined in additional scripts on the page.
...
The optimizer will combine modules that are specified in arrays of string literals that are passed to top-level <<REQUIRE_DESC,require>> and <<DEFINE_DESC,define>> calls.
****
...
Universal Module Definition (UMD) are the series of patterns / code snippets that provide compatibility boilerplate to make modules environment-independent. Those patterns can be used support multiple modules formats. Here is example of module definition that can behave as AMD module or registers itself in browser globals.

[source,javascript]
----
include::include/ch7_umd_amd_globals.js[]
----
<1> Checking if AMD loader is available. If it is, proceed with defining module according AMD spec.

<2> If AMD loader doesn't present, using factory method to instantiate object and attaching it to global object.

<3> Passing a top-level context and defining providing a implementation of a factory function.

Worth to mention that UMD is not a specification or a standard.
Ultimately, you need to care about UMD in case when your modules shall be run in a variety of environments (browser, NodeJS). In most cases, make lot of sense to choose a single module format and use it.
More information about UMD and commented code snippets for different situations could be found in https://github.com/umdjs/umd[UMD project repository].
...
The ECMAScript 6 (ES6) specification is an evolving draft outlining the changes and features in the next version of JavaScript. Very soon this specification will be finalized, but before that happens for anything introduced in ES6 browsers support will be experimental at best and cannot be used for widely-deployed web applications. One of the most important features of upcoming ES6 is the _module syntax_. Here is an example of how we can write our code:
