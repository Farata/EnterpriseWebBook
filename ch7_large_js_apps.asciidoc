...
// vik 
You'll learn pros and cons of both formats. But for our opinion, the AMD module format is suitable to play nicely with the asynchronous nature of Web. In this chapter we will use AMD module format and http://requirejs.org/[RequireJS] framework to implement modularized version of _SaveSickChild.org_.

Also, you'll see how to use RequireJS APIs to implement on-demand loading of the application components.
...
You can find more details about CommonJS, AMD and ES6 module format in the corresponding sections of this chapter.

// vik
Once we would manage how to asynchronously the load application modules, we need to allow them to communicate to each other. Straightforward approach to explicitly specify the components dependencies with work will fairy small code base and with very small amount of components. You're going to learn how to handle inter-module communications in loosely coupled fashion using the _Mediator_ pattern.

// /vik

In the next section we start reviewing the approaches and patterns of modular JavaScript architectures.
...
Ultimately, it's just some code in an IIFE that creates a module object in the internal scope of a function and exposes that module to the global scope using various JavaScript syntax. Here are couple examples of how the Module pattern could be implemented using IIFEs.
...
----
<1> Exporting to variable a module object that was created in closure.
<2> Because of JavaScript's function scoping, other parts of the code unable to access code inside closure. With this approach you can emulate private member constructs available in other object-oriented languages.
...
----
<1> Instead of exporting a module to a variable, like we did in previous example, here we passing global object as parameter of closure.
<2> Attaching the newly created object to global object. After that, `loginModule` object can be accessed from other parts of application as `window.loginModule` or just `loginModule`.
...
// vik
WARNING: The snippet above may give you an impression that all module's code is executed in global scope, but it's not. Each module is executed in its own scope which helps to isolate modules. This works automatically when you write modules for NodeJS running on the server. But to use CommonJS module format in the Web browser you need to use an extra tool to generate _transport format_ (that will be delivered and executed by browser) from _authoring format_. http://browserify.org/[Browserify] takes all your scripts and concatenates them it to one large file. Resulted bundle code, besides modules code, will contain the boilerplate code that provides CommonJS modules runtime support in the browser environment. This build step complicates the development process (the approach "_code/save/refresh browser_" doesn't work in this case anymore) and makes it harder to get started on a new project (required to install additional build tool, setup the build scripts). 
...
Now that you know how to load the module's code, let's see the module's file internals that make it consumable by the module loader.
...
Here is an example of CommonJS module format with ReuiqreJS.
...
=== Advanced Modularization Topics

* DI/IOC
* Patterns (mediator, service locator)
