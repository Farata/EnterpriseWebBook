...
We are going to use the tasks running framework for the JavaScript project called http://gruntjs.com/[Grunt]. If you're not familiar with Grunt please refer <<appendix_a>> for additional information about usage Grunt in Save Sick Child project.
...
A simple approach of arranging communication between the components is to just put all these components on the page and access their public properties directly. This would produce an application with tightly coupled components that know about each other and removal of one component would lead to multiple code changes in the application.

The better approach is to create loosely coupled components that are self-contained, do not know about one another and can communicate with the "outside world" by sending and receiving events.

Creating the UI of from reusable components applying messaging techniques requires creation of  loosely coupled components. Say you've created a window for a financial trader. This window gets a data push from the server showing the latest stock prices. When the trader likes the price he may click on the Buy or Sell button to initiate a trade. The trading engine can be implemented in a separate  window and establishing inter-window communications in a loosely coupled manner is really important. 

_Mediator_ is a behavioral design pattern that allows to unify communications of the application components. The Mediator pattern promotes the use of a single shared object that handles (mediates) the communication between other objects. None of the components is aware of the others, but each of them knows about a single object - the mediator.
...
Here is a HTML code snipped that implements described above user interface specification.
...
----
<1> The following `div` element is a container that represents _Pricing Panel_ with Bid and Ask controls.
<2> The following `div` element is a container that represents _Order Panel_ with Quantity, Go and Cancel controls.

As we described before, Mediator will handle communication between the application components. The components need to register itself with mediator so it can route communication. In a following snipper, you can simple possible Mediator implementation.
...
----
<1> A private private object that will store registered components
<2> With `Mediator.register()` function we can store component in a associative array.
<3> Setting mediator instance to registered component.
<4> Registering component in components associative array with provided `name` as key.
<5> Component can invoke `Mediator.broadcast()` when it have some information to share with the rest of the application components.
<6> If component has property, which is a function and if whose name matches the pattern `"on" + event`, e.g. `onClickEvent` Mediator will invoke this function in context of `source` object.
...
<2> Registering the components in Mediator.
<3> We're setting the `click` event listener for Bid price component.
<4> When Bid price changes Mediator will broadcast "BidClick" event to all registered component. Only component that has specific event handler, which name matches the pattern `"on" + event`, would be able to receive event.

Let's see how actual components' code looks like. Following code snippets 

[source,javascript]
----
include::include/ch7_pricepanel.js[]
----
<1> Setter of Mediator object. Mediator injects its' instance during component registration (refer to <<mediator_impl>> snippet).
<2> Getter of Mediator object.
<3> The `onBidClick` event handler. Mediator will call this function once `BidClick` event will be broadcast. Using getter `getMediator` we can broadcast `PriceUpdate` event to registered components.

[source,javascript]
----
include::include/ch7_orderpanel.js[]
----
<1> Mediator getter and setter have purpose similar to described in previous snippet.
<2> In `OrderPanel` component code we have definition of `PriceUpdate` event handler - `onPriceUpdate()`.

As you noticed, `OrderPanel` and `PricePanel` don't know about existence of each other, but nevertheless they can sent and receive data with little help of intermediate - Mediator object.
