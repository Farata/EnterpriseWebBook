...
Universal Module Definition (UMD) is the series of patterns and code snippets that provide compatibility boilerplate to make modules environment-independent. Those patterns can be used support multiple modules formats.


Here is an example of module definition using the UMD notation. In the following example, the module can be used with the AMD loader as one of variations of the Module Pattern.
...
[[REQUIREJS]]
=== Mincing Save Sick Child Application Into Modules

After covering the basics of AMD and different modularization patterns let's see how modularization can work in the real world. In this section we'll introduce the AMD-complaint module loader http://requirejs.org/[RequireJS] and will show how you can apply this framework in real world.

TIP: http://github.com/cujojs/curl[*curl.js*] is another AMD-compliant asynchronous loader. curl.js and RequireJS are very similar frameworks. But If you want to learn differences between them you can follow https://groups.google.com/forum/#!topic/requirejs/niUyLZrivgs[this thread on RequireJS group]. Interestingly, that in this thread you can find answers of the developers of RequireJS (James Burke) and curl.js (John Hann).

Let's start with brief explanation of structure of the directories for Save Sick Child application that we will use in this chapter.

[[ssc_structure]]
.A directory structure of Save Sick application for this chapter
image::fig_07_05.png[align="center"]

1. The +app/modules+ directory. We will store all application's JavaScript files here.
+

Inside modules directory we can have as many nested folder as we want. For example, here we have +utils+ folder.

2. The application assests we will left as is we had in previous chapters.
3. The +components+ directory. Here we will put all managed with Bower dependencies like RequireJS and its plugins, jQuery and etc. For more details on Bower please read <<what_is_bower>> sidebar.
4. The +dist+ directory. We will use this directory as output of optimized version of application. We will cover optimization with `r.js` later in <<using_reuiqrejs_optimizer>> section.
5. The +test+ directory. We will place the QUnit/Jasmine tests in this directory. We not going to talk about testing in this chapter. Consider it as foundation of the talk about testing techniques for JavaScript applications in the next chapter <<tdd_with_javasctipt>>.
...
// vik
Once RequireJS downloaded and placed into project directories, we need to add it to the html file like demonstrated in <<add_requirejs>> snippet.
...
----
[[data_main_attrib]]
<1> Once the RequireJS library has loaded it will look for the `data-main` attribute and attempt to load `app/config.js` script asynchronously. The `app/config.js` is the entry point of your application.

// /vik

==== Inside +config.js+
RequiredJS  uses a configuration object that includes that modules and dependencies to be managed by the framework.

[source,javascript]
----
include::include/ch7_require_config.js[]
----
<1> http://requirejs.org/docs/api.html#config[RequireJS Documentation] has comprehensive overview of all configuration options. We've included some of them here. 
...
<3> After configuring the modules' paths we're loading the 'main' module. We're kicking off the navigation of our application flow there.

// vik

==== Loading Modules On-Demand

Couple components of our application don't necessarily need to be loaded in the time when application starts. Similar to Mercedes Benz website example, some functionality of Save Sick Child website could be loaded later when user needs it. For example, user might never approach "Where we work" section. So those functionality is a great candidate for "load on-demand" module.
...
Let's apply technique demonstrated in <<load_ondemand_snippet>> snippet for Save Sich Child application to load "Who We Are", "What We Do", "Where We Work" and "Wat To Give" section only if user clicked on corresponding top bar link.
...
----
<1> As was explained earlier in this chapter, first argument of the `define` function receives array of dependencies.
+

The `!` sign in module id string literal means that RequireJS delegated some work to the plugin named `order` for example. This plugin enforces loading order. We will talk about RequireJS plugins late in this chapter in <<requirejs_plugins>> section.

<2> An array of modules that we will be loading on demand.

<3> Function that will produce click handled described in <<load_ondemand_snippet>> snippet. 

<4> In this function we instantiate click handle and apply it to button defined in module config.

<5> Finally, we need to initialize each module button with lazy loading handler.

[[requirejs_plugins]]
==== RequireJS plugins

TBD +async.js+ and +order.js+
