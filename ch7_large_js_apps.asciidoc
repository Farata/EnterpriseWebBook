...
=== Different Roads To Modularization

// /vik
Thus JavaScript has no build-in support of modules. Nonetheless developer's community managed to work around using existing syntax constructs, libraries and conventions to emulate modules-like behavior. A _Module Patter_ doesn't required any additional frameworks and work perfectly out-of-the box in any JavaScript environment. _CommonJS_ module format is widely adopted for server-side JavaSctipt, and _AMD_ format have found huge audience of adopters for a browser environment. 

Next you'll learn all this techniques in more details.
...
----
<1> This approach is similar to previous one. Here we passing global object as parameter of factory function.
<2> We attaching newly created object to global object. After that, `loginModule` object can be accessed from other parts of application as `window.loginModule` or just `loginModule`.
...
<2> Now we can logically structure application code using namespaces. Global `ssc` object will contain only code related to Save Sick Child application.
...
http://www.commonjs.org/[CommonJS] is a working group that works on design, prototyping and standardization of JavaScript APIs. They've attempted to develop standards for both modules and packages. The CommonJS module proposal specifies simple API for declaring modules, but mainly on the server-side. CommonJS module format are optimized for non-browser environments since the early days of server-side JavaScript.

On the browser side, we're always need to consider slow HTTP communication. One of the solutions is to concatenate the our code into a handful of bundles to decrease number of HTTP calls. But server-side engines ignore this step since file access is nearly instantaneous. Moreover, this is allow to dedicate each file to exactly one module for ease development, testing and maintainability.

The CommonJS specification standardizes three scoped variables: `require`, `exports`, and `module`:

[source,javascript]
----
include::include/ch7_commonjs.js[]
----
<1> If a module requires other modules, you can declare references to the other modules into the current module's scope by using the `require` function. You need to call `require(id)` for each module that your code depend on.  Module id has slashes in which is like a file path or a url to indicate namespaces for modules. The modules are grouped with related modules into a packages.

<2> The `exports` object exposes the public API of a module. All objects, functions, constructors, etc. that your module provides must be declared as properties of the exports object. The rest of the module is not exposed.

<3> The `module` variable provides metadata about module. It holds the `id` and unique `uri` of each module. The `module.export` exposes `exports` object as it's property. Because objects in JavaScript are passed as references, then `exports` and `module.exports` are the same object.

TIP: From the snippet above may seem that all module's code is executed in global scope. But it's not. Each module is executed in its own scope which helps to isolate the modules. This works automatically when you write modules for NodeJS. To use CommonJS module format in browser you need to introduce extra tool to generate _transport format_ from _authoring format_. http://browserify.org/[Browserify] takes all of your scripts and concatenates them it to one large file. But the build step complicates the development process and makes it harder to get started on a new project.
...
* The browsers can't consume CommonJS modules because browsers don't create the scoped variables `require`, `exports`, `module`. The additional build step required.
* The `require` method is synchronous, but module values may not be fully loaded during the execution.
* It's suitable for loading +js+ files, but can't load other assets like css and HTML via CommonJS APIs.

NOTE: If you want to write modules in format that can be used in the different environments, we will provide couple advices in <<umd>> section on this chapter.
