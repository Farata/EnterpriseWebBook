...
<3> The function that will produce click handled described in <<load_ondemand_snippet>> snippet. 
<4> Instantiate the click handler and apply it to button defined in module config.

<5> Finally, we need to initialize each module button with the lazy loading handler.
