...
Of course, you can load each JavaScript file using the `<script>` tag. But this approach is slow and error prone. Developer has to manage dependencies and loading order manually. Moreover, each `<script>` tag directive means additional HTTP call to the server. As the application gets larger the latency grows accordingly, which is illustrated in the following code sample.
...
In the next section, we start our journey to the approaches and patterns one can use to create modular JaveScript architecture.
...
RequreJS also supports CommonJS module format.
...
The Mediator receives the message and re-posts it toits other child - the Order Panel that knows how to place orders to purchase stocks. The main takeaway from such design is that the Pricing and Order panels do not know about each other and are communicating by sending-receiving messages to/from the mediator. Such a loosely coupled design allows reusing the same code in different applications. For example, the Pricing Panel can be reused in some portal that's used by the company executives in a dashboard showing prices without the need to place orders. Since the Price Panel has no string attached to the Order Panel, it's easy to reuse the existing code in such a dashboard.
