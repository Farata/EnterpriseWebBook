...
At this point, you should get the idea how to load module's code. Next, let's see how make your modules recognizable by RequireJS loader. 

[source,javascript]
----
include::include/ch7_module_contentLoader.js[]
----
<1> As we discussed in <<amd_section>> section, code that we want to expose as module should be wrapped in the +define+ function call. First parameter is array of dependencies. Location of dependencies files we're defined <<inside_config>>. Dependency object not necessary should have same name as dependency string id. Most important is order that dependency objects are passed to factory function. Order should be the same as order in dependencies array.

<2> in this module we export only on constructor function that returns object that uses +render+ function to draw visual component (in our case, this is part of HTML file with text for specific module) to screen. 

<3> The +contentLoader+ object loaded form +app/modules/util/new-content-loader.js+ (refer the +config.js+), instantiated by RequireJS and ready to use. 
...
RequireJS plugins are special module that implement a specific API. Plugins can extend default loader's functionality. For example +text+ plugin allows to specify text file as dependency, +cs!+ translates _CoffeeScript_ files in to JavaScript. The plugin's module name comes before the +!+ separator.

In Save Sick Child application we're using +async.js+ and +order.js+ plugins. +Async+ is a plugin for asynchronous dependency load Google Maps. +Order+ is plugin that allows specify exact order in that the dependencies should be loaded.

Full list of available RequireJS plugins you can find in https://github.com/jrburke/requirejs/wiki/Plugins[official wiki page].
...
Understanding various technologies and frameworks available in JavaScript, combined with the knowledge of different ways of linking modules and libraries is crucial for developrs who want their JavaScript applications to be more responsive.
