...
After covering the basics of AMD let's see how modularization can work in the real world. In this section we'll  introduce the AMD-complaint module loader http://requirejs.org/[RequireJS]. 
RequireJS (include info about build with `r.js`)


TIP: http://github.com/cujojs/curl[*curl.js*] is another AMD-compliant asynchronous loader. 
...
<3> After configuring the modules' paths we're loading the 'main' module. the navigation of our application flow is defined there.
