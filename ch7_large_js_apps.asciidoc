...
Finally, we're going to discuss such advanced topics as inter-module communications and Dependency Injection in JavaScript.

The code of the Save Sick Child application will be refactored to implement the module system based on RequireJS for lazy loading of the application components.
...
----
<1> This approach is similar to the previous one. Here we passing global object as parameter of factory function.
...
In brief, the CommonJS specification standardizes three scoped variables: `require`, `exports`, and `module`:
...
TIP: The snippet above may give you an impression that all module's code is executed in global scope, but it's not. Each module is executed in its own scope which helps to isolate modules. This works automatically when you write modules for NodeJS running on the server. To use CommonJS module format in the Web browser you need to use an extra tool to generate _transport format_ from _authoring format_. http://browserify.org/[Browserify] takes all your scripts and concatenates them it to one large file. But the build step complicates the development process and makes it harder to get started on a new project.
