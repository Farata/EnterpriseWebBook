...
Modules are code fragments that implement specific functionality written using a specific pattern. There is no out-of-the box modularization scheme in JavaScript language. The upcoming ECMAScript 6 specification tends to resolve this by introducing the _module concept_ in the JavaScript language itself. This is the future. Aren't .js files modules already? 

Of course, you can load each JavaScript file using the `<script>` tag. But this approach is error prone and slow. Developers have to manage dependencies and loading order manually. Moreover, each `<script>` tag results in an additional HTTP call to the server.

As the application gets larger the latency grows accordingly, which is illustrated in the following code sample.
...
----
<1> All modules depend on the code from Core jQuery library hence load it first.
...
We need a different approach to modularizing applications. Let's  start with using the Module pattern to leverage Immediately-Invoked Function Expressions (IIFE) as the first step to modularization. Next, we will introduce and compare two popular JavaScript solutions and modularization patterns - CommonJS and Asynchronous Module Definition (AMD), which are alternative approaches to modularization. Both CommonJS and AMD are specifications and sets of APIs. 

You'll learn pros and cons of both formats later in the chapter, but in our opinion, the AMD module format is suitable to play nicely with the asynchronous nature of Web. In this chapter we will use the AMD module format and http://requirejs.org/[RequireJS] framework to implement the modularized version of the Save Sick Child application.

Also, you'll see how to use the RequireJS APIs to implement on-demand loading of application components.

The upcoming ECMAScript 6 specification suggests hoe to handle modules, and how you can start using ES6 module syntax today with the help of third-party tools like http://corner.squareup.com/2013/02/es6-module-transpiler.html[transpiler]. The ES6 module syntax can be compiled down to existing module solutions like CommonJS or AMD.

For example, consider the following import code snippet from the ES6 module:

.ES6 format for importing module dependency
[source, javascript]
----
...
With the ES6 transpiler from Square team this code could be translated into the following:

.Module defined in AMD format
[source,javascript]
----
define("main",
  ["login"],
...
.Module defined in CommonJS format
[source,javascript]
----
"use strict";
...
You can find more details about CommonJS, AMD and the ES6 module format in the corresponding sections of this chapter.

After application modules are asynchronously loaded, they need to communicate to each other. You can explicitly specify the components dependencies, wwhich work will fine if you have a handful of  components. A more generic approach is to handle inter-module communications in a loosely coupled fashion using the _Mediator_ pattern, CommonJS or AMD formats. In the next section we start reviewing the approaches and patterns of modular JavaScript architectures.

=== Roads To Modularization


JavaScript has no build-in language support of modules. Nonetheless developers' community managed to find a way for modularization using existing syntax constructs, libraries and conventions to emulate modules-like behavior. The _Module pattern_ doesn't require any additional frameworks and works in any JavaScript environment. The _CommonJS_ module format is widely adopted for server-side JavaSctipt, and the  _AMD_ format is very popular in applications that run in Web browsers. In this section you'll learn all these techniques in  details.


==== The Module Pattern

In software engineering, the Module pattern was originally defined as a way to implement encapsulation of reusable code. In JavaScript, the Module pattern is used to emulate the concept of classes. We're able to include both public/private methods and variables inside a single object, thus hiding the encapsulated code from being visible from global scope. Such encapsulation lowers the likelihood of conflicting function names defined in different scripts, but used in the same application's scope.

Ultimately, it's just some code in an IIFE that creates a module object in the internal scope of a function and exposes that module to the global scope using JavaScript syntax. Here are couple examples of how the Module pattern could be implemented using IIFEs.
...
.Piggiybacking on the global object
[source,javascript]
----
include::include/ch7_iife_global_loginModule.js[]
----
<1> Instead of exporting the module to a variable as in the previous example, here we're passing the global object as a parameter of the closure.
...
----
<1> Here we have modification of an approach described in previous snipped. Now we can create namespace for our application called `ssc`.
...
This approach doesn't scale well because when working with an application with the large number of modules, very soon you may find yourself adding lots of boilerplate code checking objects' existence in the global scope for each new module. Also, you need to be careful with managing namespaces: since you are the one who put an object into the global scope, you need to think  how to avoid accidental names conflicts.

The Module pattern has a serious drawback - we still need to deal with manual dependency management and manually arrange `<script>` tags in the HTML document.

==== CommonJS


http://www.commonjs.org/[CommonJS] is an effort to standardize JavaScript APIs. People who work on CommonJS APIs have attempted to develop standards for modules and packages too. The CommonJS module proposal specifies a simple API for declaring modules, but mainly on the server-side. The CommonJS module format was optimized for non-browser environments since the early days of server-side JavaScript.

On the Web browser side, you always need to consider potentially slow HTTP communications. One of the solutions is to concatenate al your scripts into a handful of bundles to decrease number of HTTP calls, which was not a concern for the server-side JavaScript engines because file access is nearly instantaneous. Moreover, separation of the code allowed to dedicate each file to exactly one module for ease development, testing and maintainability.

In brief, the CommonJS specification standardizes http://wiki.commonjs.org/wiki/Modules/1.1#Module_Context[three free variables]: `require`, `exports`, and `module`:

[source,javascript]
----
include::include/ch7_commonjs.js[]
----
<1> If a module requires other modules, declare references to those modules inside the current module's scope by using the `require` function. You need to call `require(id)` for each module that your code depends on.  The module id has slashes defining the file path or a url to indicate namespaces for modules. The modules are grouped with related modules into a packages.

<2> The `exports` object exposes public API of a module. All objects, functions, constructors that your module exoses must be declared as properties of the exports object. The rest of the module is not exposed.

<3> The `module` variable provides metadata about module. It holds the `id` and a unique `uri` of each module. The `module.export` exposes `exports` object as it's property. Because objects in JavaScript are passed as references, then `exports` and `module.exports` are the same object.

WARNING: The snippet above may give you an impression that all module's code is executed in global scope, but it's not. Each module is executed in its own scope which helps to isolate modules. This works automatically when you write modules for NodeJS running on the server. But to use CommonJS module format in the Web browser you need to use an extra tool to generate _transport format_ (to be delivered and executed by the browser) from _authoring format_. http://browserify.org/[Browserify] takes all your scripts and concatenates them it to one large file.  Besides modules' the resulted code will contain the boilerplate code that provides CommonJS modules runtime support in the browser environment. This build step complicates the development process (the approach "_code/save/refresh browser_" doesn't work in this case) and makes it harder to get started on a new project as you need to install the additional build tool and setup the build scripts. 
...
* Browsers can't work with CommonJS modules because browsers don't automatically create the scoped variables `require`, `exports`, `module` hence the additional build step is required.
...
NOTE: If you want to write modules in the format that can be used in the different environments read our suggestions in the  <<umd>> section on this chapter.
...
AMD began as a draft specification for module format in CommonJS, but since the full agreement about its content was not reached, the further development of the module format moved to the https://github.com/amdjs[amdjs Github page].
...
----
<1> In this example, we are not providing the optional +module_id+. The file name without the `.js` extension will be used as +module_id+. Our module has one dependency on the module called +newContentLoader+. The dependent module instance will be passed in the factory method as variable +contentLoader+. 
...
* A CommonJS module authoring format is supported by majority of loaders. You'll see example of modules later in <<require_cjs>> section. 
...
----
<1> In real-world enterprise applications array of dependency modules might be very big.

* Easy to get a mismatch between dependency array and callback arguments.
+
...
----
<1>  Module IDs and factory function arguments mismatch will cause module usage problems.
...
Universal Module Definition (UMD) is a series of patterns and code snippets that provide compatibility boilerplate to make modules environment-independent. Those patterns can be used support multiple modules formats. Here is an example of module definition using the UMD notation. In the following example, the module can be used with the AMD loader as one of variations of the Module Pattern.
...
It's worth mentioning that UMD is not a specification or a standard. You need to pay attention to UMD patterns in case when your modules shall run in more than one type of environment (e.g. a browser and on the server side engine running NodeJS). In most cases, it makes a lot of sense to use a single module format.
...
****
Since ES6 standard features not yet implemented by a wide majority of browser manufactures you can use third party tools to get a flavor of upcoming enhancements in JavaScript language. http://corner.squareup.com/2013/02/es6-module-transpiler.html[ES6 transpiler] developed by http://squareup.com/[Square] Engineers helps to use module authoring syntax from ES6 and compile it down to transport formats that you learned earlier in this chapter.
...
[source, javascript]
----
export function area(radius) {  // <1>
  return Math.PI * radius * radius;
}

export function circumference(radius) { // <1>
  return 2 * Math.PI * radius;
}
----
<1> A module exports two functions: `area` and `circumference`.

.A `main.js` that uses fucntions from `circle.js` module
[source, javascript]
----
...
----
<1> A `import` keyword uses to specify that objects we want to use from module.
<2> Here we're using imported functions.
...
----
<1> With `type` command line switch we can specify that output format will be: _amd_, _cjs_ and _globals_.
...
If you not sure that you need or don't need to check your JavaScript dependencies under source control, read the http://addyosmani.com/blog/checking-in-front-end-dependencies/[article by Addi Osmani] that covers different aspects of this question.
...
Once we delegated the modules loading to RequireJS let's see the module's file internals that make it consumable by the module loader.
