...
----
<1> All modules depend on the code from Core jQuery library. We need to load it first.
...
<3> Main application script should be loaded after all dependencies.
...
http://www.commonjs.org/[CommonJS] are a volunteer working group which aim to design, prototype and standardize JavaScript APIs. They've attempted to develop standards for both modules and packages. The CommonJS module proposal specifies a simple API for declaring modules mainly on the server-side.
...
// provide the example
Each module executed in its own scope
How to make that code available if it's not in global scope?

`exports`
The exports variable is an object that is the implicit value of your module.
When your module is requested, the code is executed and then exports is returned for you.

CommonJS pros:

* Simple API. Easy to write and use modules.
* This modules organization pattern is widespread in server-side JavaScript, e.g. NodeJS.

CommonJS cons:

* Additional build step required. We will discuss this in <<Browserify>> section.
* The `require` method is synchronous, but module values may not be fully loaded during execution.
* Suitable for loading +js+ files. Not possible to load other assets via CommonJS APIs.
