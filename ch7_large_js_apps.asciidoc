...
Of course, you can load each JavaScript file using the `<script>` tag. But this approach is error prone and slow. Developer has to manage dependencies and loading order manually. Moreover, each `<script>` tag directive means additional HTTP call to the server and browsers have limited number of simultaneous connections to one host.
...
// vik
For example, consider following snippet of ES6 module import syntax:
...
Using ES6 transpiler form Square team this code could be translated in following:
...
More details about CommonJS, AMD and ES6 module format you can find in corresponding sections of this chapter.
...
// vik
In the next section, we start our journey to the approaches and patterns of modular JavaScript architectures.
...
RequreJS also supports CommonJS module format with slightly different signature of `define()` function. That helps bridge the gap between AMD and CommonJS. If your factory function accepts parameters, but you omit the dependency array, the AMD environment assumes you wish to emulate a CommonJS module environment. The standard `require`, `exports`, and `module` variables are injected as parameters to the factory.
...
----
<1> Factory receives up to three arguments that emulate the CommonJS `require`, `exports`, and `module` variables. 

<2> You should export your module, rather than return it. There are two ways how to export object - assign the the module directly to `module.exports` (shown in this snippet)or set properties on the `exports` object. There are use cases for each export mechanism, but that is out of a scope of this chapter.

<3> In CommonJS, dependencies are assigned to local variables using `require(id)` function.
...
A _Mediator_ is a behavioral design pattern that allows to unify a way how the different components of a application can communicate. The Mediator Pattern promotes the use of a single shared subject that handles communication with multiple objects. Basically, all components are not aware about rest of the components and communicate with each other through the mediator. By placing mediators, communication can be handled through a single object, rather than having multiple objects communicating with each other.

In chapter one we introduced example of trader's desktop. Let's reuse same example and refactor it with Mediator object. 

The Pricing Panel on the left gets the data feed about the current prices of the IBM stock. When the user clicks on Bid or Ask panel, the Pricing Panel just sends the event with the relevant information like JSON-formatted string containing the stock symbol, price, buy or sell flag, date, etc.
...
As you can notice, introduction of a mediator increases the reusability of the components by decoupling them from the system. A mediator simplifies maintenance of the system by centralizing control logic, reduces the variety of messages sent between objects in the system. A drawback of the Mediator pattern is that without proper design, the Mediator object itself can become overly complex.
