...
There are many testing frameworks in the JavaScript world, but we'll give you a brief overview of two of them: http://qunitjs.com/[QUnit] and http://bit.ly/1pPx7lo[Jasmine]. The main goal of each framework is to test small  pieces of code, a.k.a. _units_.

We will go through basic testing techniques known as test-driven development and Test First. You'll learn how to automate the testing process in multiple browsers with http://bit.ly/1vnEqCj[Testem Runner] or by running tests in so-called _headless_ mode with http://phantomjs.org[PhantomJS].
...
****
Although _quality assurance_ (QA) and _user acceptance testing_ (UAT) are far beyond the scope of this chapter, you need to understand their differences.((("test-driven development (TDD)", "quality assurance vs. user acceptance testing")))((("quality assurance (QA)")))((("user acceptance testing (UAT)")))
...
PhantomJS is built on top of WebKit and http://bit.ly/1iSSuDw[JavaScriptCore] (like Safari), and SlimerJS is built on top of Gecko and http://mzl.la/1kXXRkc[SpiderMonkey] (like Firefox). You can find a comprehensive list of differences between PhantonJS and SlimerJS APIs in http://bit.ly/1nfFoxq[SlimerJS's documentation site].

In our case, Grunt automatically spawns the PhantomJS instance, executes the code of our tests, reads the execution results using the PhantomJS API, and prints them out in the console. If you're not familiar with Grunt tasks, refer to the http://bit.ly/1iJO41S[online bonus chapter] for additional information about using Grunt in our Save The Child project.
****
...
Refer to <<WHAT_IS_PHANTOMJS>> to familiarize yourself with http://phantomjs.org[PhantomJS]. The slide deck titled http://bit.ly/1vsVeK5["Browser Performance metering with PhantomJS"] is yet another good resource for seeing how you can use PhantomJS for performance testing.((("test-driven development (TDD)", "PhantomJS")))(((PhantomJS)))
...
An _assertion_ is a true-false statement that represents what a programmer assumes about program state. For example, +customerID >0+ is an assertion. According to http://bit.ly/UA6xDb[Martin Fowler], assertion is a section of code that assumes something about the state of the program. Failure of an assertion results in test failure.(((assertions))) 
...
[[AAAR]]
This pattern for authoring unit tests is called _arrange-act-assert-reset_ (http://bit.ly/TJiWUI[AAAR]).((("test-driven development (TDD)", "AAAR pattern")))((("AAAR (arrange-act-assert-reset)")))
...
You need to get only two files: __qunit.js__ and __qunit.css__, as shown in <<fig_qunit_framework>>.
...
<1> Grunt loads the task from the local npm repository. To install this task in the __node_modules__ directory, use the command +npm install grunt-contrib-qunit+.
...
<5> Typically, you need to use assertions to make sure the code being tested gives expected results. The function http://bit.ly/1r5x3Sn[`ok`] examines the first argument to be `true`. 
...
You can find more details about QUnit in its http://api.qunitjs.com[product  documentation] and http://bit.ly/1vnELF6[QUnit Cookbook].         
...
image::images/ewdv_0705.png[float="true", scale="50"]
...
image::images/ewdv_0706.png[scale="65"]
****
...
<2> The function `it()` also accepts similar parameters--the name of the test specification, and the function that implements this specification. As with suites, Jasmine has a corresponding `xit` method to exclude the specification from execution.
...
Here, we've listed only some of the existing matchers. You can find the complete documentation with code examples at the http://bit.ly/1mzA3if[official Jasmine website] and http://bit.ly/100yWTt[wiki].

TIP: A large set of jQuery-specific matchers are available at link:$$https://github.com/velesin/jasmine-jquery$$.
...
Consider the login functionality shown in <<LOGIN_SNIP>>. A `showAuthorizedSection()` function will be invoked within the `login` function after the user enters the correct username and password. We need to test that the invocation of `showAuthorizedSection()` is happening in this sequence.

[[LOGIN_SNIP]]
.Production code of the login function
====
[source,javascript]
----
var ssc = {};
...
Testem will just pick any JavaScript file in your project directory. If Testem can identify any test among the _.js_ files, it will run it. But Testem tasks can be customized by using a configuration file(((Testem, configuration file))) 

You can configure Testem to specify which files should be included in testing. Testem starts by trying to find the configuration file _testem.json_ in the project directory. A sample _testem.json_ file is shown in <<TESTEM_JSON>>. 
...
<1> The `framework` directive is used to specify the test framework. Testem supports QUnit, Jasmine, and many more frameworks. You can find a full list of supported frameworks on the http://bit.ly/1mNbCTe[Testem GitHub page].
...
Testem uses http://bit.ly/V28HvG[TAP] format to report test results. 
...
The jQuery selectors API is more convenient for working with the DOM than a standard JavaScript DOM API. But in future examples, we will use the http://bit.ly/1sRcgDK[jasmine-fixture] library for easier setup of the DOM fixture. Jasmine-fixture uses syntax that is similar to that of jQuery selectors for injecting HTML fixtures. With this library, you will significantly decrease the amount of repetitive code while creating the fixtures.
...
<1> By using the `affix()` function provided by the _jasmine-fixture_ library and expressiveness of CSS selectors, we can easily set up required DOM elements. You can find more examples of possible selectors at the http://bit.ly/TJjeLl[documentation page] of jasmine-fixture.
...
<3> This is our skeleton Ext JS application that will set up a "friendly" environment for components under the test. You can see the content of _test.js_ in <<SKELETON_EXTJS_APP>>.
...
<3> We're not interested in the view's interaction--only the contract should be tested. At this point, some methods can be substituted with the fake implementation to let the test pass. In this case, the specification tests the situation when the form is valid.
...
[TIP]
====
Another UI testing tool is worth mentioning, especially in the context of testing Ext JS applications: http://bit.ly/1pEdDo5[Siesta]. Siesta allows you to perform testing of the DOM and simulate user interactions. Written in JavaScript, Siesta uses unit and UI testing. There are two editions of Siesta: lite and professional. 
====
