...
[[AAAR]]
This pattern for authoring unit tests is called _arrange-act-assert-reset_ (http://integralpath.blogs.com/thinkingoutloud/2005/09/principles_of_t.html[AAAR]).((("test-driven development (TDD)", "AAAR pattern")))((("AAAR (arrange-act-assert-reset)")))
...
<5> Typically, you need to use assertions to make sure the code being tested gives expected results. The function http://api.qunitjs.com/ok/[`ok`] examines the first argument to be `true`. 
