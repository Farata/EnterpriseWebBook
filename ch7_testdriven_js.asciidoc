...
PhantomJS is built on top of WebKit and http://trac.webkit.org/wiki/JavaScriptCore[JavaScriptCore] (like Safari), and SlimerJS is built on top of Gecko and http://mzl.la/1kXXRkc[SpiderMonkey] (like Firefox). You can find a comprehensive list of differences between PhantonJS and SlimerJS APIs in http://docs.slimerjs.org/current/differences-with-phantomjs.html[SlimerJS's documentation site].
