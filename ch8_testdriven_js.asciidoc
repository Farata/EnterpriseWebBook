...
.What is PhantomJS?
****
PhantomJS is a headless WebKit with JavaScript API. Webkit is the rendering engine that Safari, and a few other browsers use. PhantomJS is a browser, but this browser doesn't have any graphical user interface (GUI) element. PhantomJS can execute HTML, CSS, JavaScript code. Because it's not required to render GUI PhantomJS can be used in display-less environments (such as Continuous Integration server) to run the tests. In our case, Grunt automatically spawn PhantomJS instance, executes our the code of our tests, reads execution results using PhantomJS API and prints it out into the console.
****
...
Every specification file has set of _suites_ defined in `describe` function. Suite helps logically organize code of your specifications. 

.HelloSpec.js
[source,javascript]
----------------------------------------------------------------------
describe("My test suite", function(){       // <1>
    it("my first spec", function(){         // <2>
        // place specification code here
        // 
    });
    it("my another spec", function(){       // <3>
        // 
    });
});
----------------------------------------------------------------------

<1> Function `describe` accepts two parameter - name of suite, and function. The function is a block of code that implements the suite.
<2> Function `it` accepts two parameters - name of specification, and function that implements specification.
<3> Each suite can have any number specs.

// TBD

.Specifications Anatomy TODO
* suite
** `describe` defines suite
** `xdescribe` exclude suite from execution
* spec
** `it` defines specification
** `xit` exclude specification from execution
* `expect` 
