...
<1> Register new task named `hello`.
<2> Print string using http://gruntjs.com/api/grunt.log[grunt's log API]

Let's run this `hello` task with command `grunt --gruntfile Grunt_simple.js hello`
...
The idea behind _behavior-driven development_ (BDD) is to use "natural language" constructs to describe what you think your code should be doing, or more specifically, what your functions should be returning.
Similar to unit tests, with BDD you write specifications that are small and and test one thing at the time. Specifications should be sentences. For example, "Calculator add two positive numbers". It will help you easy identify failed test by simply reading this sentence in test results report. As you will see next in this chapter, BDD framework provide very nice way to group, execute, and report JavaScript unit tests. 
Next, we're going to show how to use Jasmine - BDD framework for JavaScript.

===== Setup Grunt with Jasmine

You've learned already how to use Grunt tool to automate boilerplate task. Next in this section we're going to show how you can execute Jasmine specification with Grunt.
...
.Running Jasmine Specs in Browser
[[FIG-2]]
image::images/fig_08_02.png["SpectRunner.html"]

+SpecRunner.html+ file is very similar to qunit html runner. You can run specs by opening runner file in browser.  
...
<4> Following code initialize Jasmine and run all specs when page gets loaded.

Now let's update our Grunfile to run same sample specs with headless browser. For that we need to copy content of +src+ folder of your Jasmine distribution into `app/js` folder of our project and content of `spec` folder into `test/spec` folder of our project. Also create folder `test/lib/jasmine` and copy there content of Jasmine distribution `lib` folder. <<FIG-3>>

.Jasmine sample specs in our project structure
[[FIG-3]]
image::images/fig_08_03.png[]

Now we need to edit +Gruntfile.js+ to activate Jasmine support.
...
<2> Specifying location of source files.
<3> Location of Jasmine specs.
<4> Location of Jasmine helpers. We will cover helpers later in this chapter.

We can run command `grunt --gruntfile Gruntfile_jasmine.js jasmine` to execute tests. Here is output of this command.
...
As we can see, grunt successfully executed tests with PhantomJS of 5 specs defined in PlayerSpec.js. In next section we will learn how write our own specs.

===== Jasmine basics

After we've set up tools to run the tests it's about a time to start developing our tests and learn Jasmine framework constructs.
