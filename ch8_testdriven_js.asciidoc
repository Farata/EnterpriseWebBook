...
After we've set up the tools for running the tests let's start developing our tests and learn the Jasmine framework constructs. Every specification file has a set of _suites_ defined in the `describe` function. Such a suite helps logically organize the code of test specifications. 
...
<2> The function `it` accepts two parameters - the name of the test specification, and the function that implements this specification. Like in case with suites, Jasmine has paired `xit` method to exclude specification from execution.

<3> Each suite can have any number of suites.

<4> Or any number of specifications.

<5> Expectations are built with the function `expect` which takes a value, called the actual. It is chained with a Matcher function, which takes the expected value.

In the previous example, you were checking to see if `2+2` equals `4`. We used the function toEqual(), which is a _matcher_. This basically takes the argument to the expect function and checks to see if it satisfies some criterion in the matcher. There are different flavors of matchers shipped with Jasmine framework. Here we're going to couple the most important and frequently used matchers functions.
...
Function `toContain()` check if actual value is an element of array.
+
...
It's frequently useful to reverse matchers to make sure that they aren't +true+. To do that, simply prefix things with `.not`:
+
...
As was mentioned above, here we listed only part of existing matchers. Full documentation with code example you can find on http://pivotal.github.io/jasmine/#section-Matchers[official Jasmine website] and on https://github.com/pivotal/jasmine/wiki/Matchers[wiki].
...
Jasmine framework in very extendable and it allows you define your own matchers if for some reasons you unable to find satisfied matchers in Jasmine distribution. In this case you can write custom matcher. Let's write matcher that check if string contains name of Secret Agent from defined list of agents. 
...
<1> `this.actual` contains value that developer passes in `expect` function.

<2> We can customize error message in case of failed test.

<3> This function checks is agentsList contains the actual value.

And this is how call of this helper is looks like:
...
And we can see custom failure message on the console.
...
<1> Custom failure message. Also we can see that failed `expect` was fourth one (4) (in the order mentioned in spec).
