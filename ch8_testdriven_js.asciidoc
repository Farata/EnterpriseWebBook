...
// <vik>
Finally, you need to clean up after the test will run - remove everything that you created during setup of the environment.
...
// <vik>
Jasmine framework has API to arrange (see <<AAAR>>) your specification. There are two methods - `beforeEach()` and `afterEach()`. They allow you to execute some code before and after each spec respectively. It's very useful for instantiation of the shared objects or cleaning up after the tests.
If you need to fulfill your test with common dependencies or setup the environment just place code inside `beforeEach()` method. These dependencies and environment knows as fixture.
...
<2> In `beforeEach` method we create two input fields. This two inputs will be available in all specification of this suite.

<3> You can place additional cleanup code inside `afterEach` function.

<4> A `beforeEach` functions helps to implement _Don't repeat yourself_ principle in the out tests. We don't need to create the dependency elements inside each spec manually.

<5> We can change defaults inside each spec without worrying to affect other specs. A environment will be reset for each specification.
...
// <vik>
As we showed earlier, Jasmine has `beforeEach()` which is suitable place to setup all required DOM elements and make them available in the specifications.
...
----
<1> Inside `beforeEach` method we're using the API to manipulate the DOM programmatically. Also, if you're using a HTML tests runner, you can add the fixture using HTML tags. But we would not recommend this approach. Very soon you will find you HTML rest runner full of fixture HTML code and your HTML runner code very soon will become unmaintainable.

<2> We create input element using jQuery APIs. This code will create corresponding input element, e.g.   
...
The jQuery API is slightly more convenient than plan DOM API. But in future examples we will use https://github.com/searls/jasmine-fixture[jasmine-fixture] library for easy setup of DOM fixture. Jasmine-fixture uses similar to jQuery selectors syntax for injecting HTML fixtures. This this library you will significantly decrease amount of repetitive code while creating the fixtures.

// TODO: add jasmine-fixture setup, how to download and add to jasmine Grunts's task

Let's see how example from <<FIX-1,previous code snippet>> looks like with jasmine-fixture library.
...
----
<1> Using the `affix` function provided by jasmine-fixture library and expressiveness of the CSS selectors we can easily setup required DOM elements. More examples of possible selectors could be found in https://github.com/searls/jasmine-fixture#more-examples[documentation page] of jasmine-fixture.

<2> Now when all requirements for our production code (`login` function) are satisfied we can run it in context of a test and assert the results.

As you can see, a testing of the DOM manipulation code is much like a other type of the unit testing. We need to prepare out fixture - the testing context, run the production code and assert the results.
