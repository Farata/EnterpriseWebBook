...
Now after you have learned of basics of Grunt tool we can use it something more interesting than just printing "_hello world_" string on the screen. 
As you already learned earlier in this book, JavaScript is a interpreted language and there is no compile to help catch syntax errors. Next we will show how you can take advantage of http://www.jshint.com/[JSHint] - the open source tool which helps you to identify errors in your JavaScript code. 
Consider following JavaScript <<LISTING_WITH_ERRORS,code>>.
...
<3> Here is another one typical typo -  a missing semicolon. It is not actual error and automatic semicolon insertion (ASI) will get you covered in this case. But omitting semicolon is not a good habit.

A previous code snippet is a fairly simple example but can cause a serious trouble and a frustration if you don't have proper tools to check the code semantics and syntax. 

Let's see how JShint can help with it. First, we need to install JSHint tools on the computer. As many JavaScript open source projects today, JSHint can be installed via NPM with command `npm install jshint -g`. 
Now we can run JSHint against our code snippet:
...
2 errors            // <2>
----
<1> JSHint provides location of error and a short description of the problem with your code.

<2> A total count of errors occurred. 
...
The Grunt also has task to run JSHint against your JavaScript code base. Here is how JSHint configuration in Grunt looks like.
...
Another handy task that we will use in our setup is `watch` task. A purpose of this task is to monitor defined in the task configuration files locations. When it detects any changes in that files it will run configured task. Here is how a <<LIST_WATCH_TASK,watch task config>> looks like
...
<3> A array of tasks that will be triggered after file change event occurs.
...
Later in this chapter we will use watch task to automatically run tests after production code or the code of the tests is changed.

In this section we introduced new tool - Grunt - task runner. Grunt can be used to automate different aspects for your JavaScript development workflow. We will use Grunt to perform syntax and semantic checks of your code with JSHint. Also, to minimize repetitive tasks we showed hot to setup `watch` task to monitor changes in our codebase.
In next sections we will dive into the testing frameworks for JavaScript. Our fist stop - QUnit.
