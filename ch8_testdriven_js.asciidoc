...
With any testing framework, your tests will follow same basic pattern. First, you need to setup up the test environment. Second, you run the  production code and check that it works as it supposed to. Finally, you need to clean up after yourself.
...
// <vik>
Jasmine framework has API to arrange (see <<AAAR>>) your specification. There are two methods `beforeEach()` and `afterEach()`. They allow you to execute some code before and after each spec. This can be very useful for instantiation common object or cleaning up after tests.
If you need to fulfill your test with common dependencies or setup the environment just place code inside `beforeEach()` method. 
...
<4> We don't need to create the dependency elements inside each spec manually.

<5> We can change defaults inside each spec without worrying to affect other specs because environment will be reset for each specification.
...
Jasmine has beforeEach(). Create with cre
