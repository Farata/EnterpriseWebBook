...
Testem supports two running mode: development mode () and continuous integration. In the development mode, testem will start a development server.

.Testem development mode.
image::fig_08_13.png[]

This server's URL should be opened in any browser that you want run test against.

image::fig_08_14.png[]

You can connect to the testem server any browser from network. For example, the <<TESTEM_IE, following screenshot>> shows Internet Explorer 10 running under Windows 7 virtual machine connected to the testem server. 
