...
// vik
In this chapter you will learn the developer's practices to perform testing of the software and we will discuss the following types of testing: 
...
****
Although, _Quality Assurance_ (QA) and _User Acceptance Testing_ (UAT) is far beyond the scope of this chapter, we believe that you need to know it and understand the difference.
Software QA (or _Quality Control_ (QC)) is the process that helps identify the correctness, completeness, security and quality of developed software. QA testing is performed by the specialists (testers, analysts). Goal of QA is that the application must comply with a set of the behavior requirements.
UAT (or simply Acceptance Testing) performed by business user or subject area expert of the application. A result of UAT is a endorsement from a end-use of tested applications/functionality/module that modification or new functionality meets agreed requirements. The results of UAT  give confidence to the end-user that the system will perform in production according to specification.
During QA process specialist intent to perform all test with attitude to break the  application. This approach helps to find undiscovered during development errors.
On the contrary, during UAT process user intent to run day-to-day scenarios and make sure that application is perform real-life business functions.
****
...
PhantomsJS is built on top of Webkit and http://trac.webkit.org/wiki/JavaScriptCore[JavaScriptCore] (like Safari) and SlimerJS is built on top of Gecko and https://developer.mozilla.org/en-US/docs/Mozilla/Projects/SpiderMonkey[SpiderMonkey] (like Firefox). Many of its differences come from differences between Webkit and Gecko platforms. You can find comprehensive list of difference between PhantonJS and SlimerJS APIs in http://docs.slimerjs.org/current/differences-with-phantomjs.html[SlimerJS's documentation site].
In our case, Grunt automatically spawns PhantomJS instance, executes the code of our tests, reads the execution results using PhantomJS API, and prints them out in the console. If you're not familiar with Grunt tasks, please, refer to <<appendix_a>> for additional information about usage Grunt in Save Sick Child project.
****
