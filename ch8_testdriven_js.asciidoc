...
In previous section of this chapter we showed you how to execute your test and specification in headless mode using PhantomJS. This is very useful to run test in Continuous Integration environments. As we learned in this chapter, PhantomJS uses WebKit engine. But it's few more browser out there - Safari, Google Chrome, Google Chrome Canary, Firefox, Internet Explorer. Definitely, manually run tests in each browser is tedious and not productive.
To automate testing in all this myriad of browsers Karma (previously known as _Testacular_) framework emerged. In this section we will show how to configure Karma and integrate it with Grunt.

===== Karma Configuration file

Karma runner is driven by configuration file.

// TBD

.Simple Karma configuration file
----
basePath = '';      // <1>
...
singleRun = true; //<5>
----
<1> Path that will be used to resolve files <2> and exclude <3>.
<2> Array of files that will be loaded in the browser.
<3> Array of files that will be excluded from loading.
<4> Array of browsers that will be used to execute JavaScript code.
<5> if true, it capture browsers, run tests and exit.
...
----
<1> Karma task is defined in grunt config.
<2> We specify config file for karma task.

Now we can run our test simultaneously on all browsers that we have on out computer - Google Chrome (+ Canary version), Safari, Firefox, Opera and PhantomJS with one command:
`grunt karma`.
