...
// Will use Casper.js as an example to demonstrate functional testing of web apps
...
****
Node.js (or simply Node) is a server-side event-driven framework. Node uses V8 - JavaScript engine that Google uses in Chrome/Chromium browsers - under the hood and provides JavaScript API for accessing a file system, sockets and  processes. Grunt tool build on top of Node JavaScript APIs. You can find more information about Node on the http://nodejs.org/about/[official website]

Node Package Manager (NPM) is utility and https://npmjs.org/[community repository] for open source JavaScript projects. NPM provides unified API and metadata model for managing dependencies in JavaScript projects. A `package.json` file is a project's dependencies descriptor. NPM installs project dependencies using information from `package.json`.

Node and NPM are cross-platform software and binaries available for Windows, Linux and Mac OS X operating systems. 
To use examples in you need to download and install Node from http://nodejs.org/download/[official web site].
After that you need to follow http://gruntjs.com/getting-started[ the instruction from official website] to install Grunt on your machine.
****

Let's start with the simplest Grunt project setup. The following two files must be present in the project directory:

* `package.json`: This file is used by Node Package Manager (https://npmjs.org/[NPM]) to store metadata and a project dependencies. 
+
You will list Grunt and its plugins your project needs as devDependencies in this file.
...
----
<1> We want to define array that contains names of actors who played James Bond in a canonical TV series.
...
****
In JavaScript, you can omit the semicolon between two statements written on separate lines. A automatic semicolon insertion is a source code parsing procedure that infers omitted semicolons in certain contexts, inserting the semicolon into the program for you. You can read more about optional semicolons in JavaScript in the chapter "Optional Semicolons" of  <<flanagan,'JavaScript. Definitive Guide. 6th Edition'>> book.
****
// /vik
...
==== Test-drive ExtJS version of _SaveSickChild.org_

// <vik>
WARNING: This section is under active development

In this section we will leverage previous knowledge about ExtJS framework from previous chapter. As you already know, ExtJS framework provides programming model based on MVC pattern. Separation of responsibilities between Views, Models and Controllers makes an ExtJS application a perfect candidate for unit testing. 
We will the ExtJS version of "Save Sick Child" application that we developed in Chapter 6.

//TBD
===== Harnessing ExtJS application

Let's create a fake application that can provide for out classes under the test familiar environment. 
...
----
<1> We need add Jasmine framework dependencies.
<2> Also we need to add ExtJS framework dependencies.
<3> Here is our fake ExtJS application that will setup "frendly" environment for components under the test. You can see content of `test.js` on <<FAKE_EXTJS_APP,following listing>>.
...
----
<1> ExtJS loader need to know about location of the testing classes...
<2> ... and about location of production code.
<3> We create fake application in namespace of production code to be to provide execution environment for production code.
<4> The `AllSpec` class will be responsible for requesting for load the rest of the specs. We will show code of `AllSpec` class in <<ALL_SPEC,next listing>>
<5> A fake application will test the controllers from the production application code.
...
----
<1> A `requres` property contains array of Jasmine suites. All further tests will be added to this array. ExtJS framework will be responsible for loading and instantiation all test classes.
...
----
<1> We wrap Jasmine suite into ExtJS class.
<2> Rest of the code is very similar to that you've already learned in this chapter about Jasmine.
