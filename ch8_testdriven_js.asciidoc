...
<3> Include the specification code. It's not required but files that contain specification code can have suffix `*Spec.js`.
...
"Austin Powers is not a secret agent (4)" is a custom failure message. Also, the failed `expect` was the fourth one (4) according to the order mentioned in spec.
