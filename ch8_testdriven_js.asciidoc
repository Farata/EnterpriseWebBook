...
// <vik>
Here is couple tools that you want look to perform functional testing of your web application.

* http://docs.seleniumhq.org/[Selenium ]
+

Selenium is advanced browser automation tool. Selenium tool suite has capabilities to run and record user scenarios without learning any scripting languages. Also Selenium has integration with many programming languages like Java, C# and etc. Selenium uses technology called WebDriver to talk to the browsers and receive running context information. WebDriver is becoming https://dvcs.w3.org/hg/webdriver/raw-file/default/webdriver-spec.html[the standard API for browser automation]. Selenium has wide range of http://docs.seleniumhq.org/about/platforms.jsp[supported browsers and platforms].

* http://casperjs.org/quickstart.html[Casper.js]
+

CasperJS is a scripting framework written on JavaScript. CasperJS allows to create the interaction scenarios like defining and ordering the navigation steps, filling and submitting the forms or even scrapping web content and making the page screenshots. CasperJS works on the top of PhantomJS. That is why CasperJS is limited use WebKit only as runtime environment. But it's very useful then you want to run the tests on Continuous Integration environment. You can read more about <<WHAT_IS_PHANTOMJS,PhantomJS>> and <<WHAT_IS_CI,CI>> later in corresponding sidebars later in this chapter.
...
// <vik>
Here is bunch of tools that you would like to check for the purpose of load testing.

* http://httpd.apache.org/docs/2.2/programs/ab.html[Apache Benchmark]
+

Apache Benchmark is command line tool and it's fairy simple to use. For example, with a command `ab -n 10 -c 10 -t 60 http://savesickchild.org:8080/ssc_extjs/` Apache Benchmark will open 10 concurrent connection and in each connection will send 10 requests (this simulates the number of visitors to your webapp) for 60 sec. Number of concurrent connections are the actual number of users that are using the application at exactly the same time. You can find a sample report in <<AB_REPORT,following snippet>>.
...
PhantomJS is headless WebKit-based browser. Please, refer the sidebar called <<WHAT_IS_PHANTOMJS,What is PhantomJS>> to make yourself familiar with this tool. You can read more how you can use PhantomJS for performance testing our your application in presentation called http://wesleyhales.com/slides/html5devconf2013/["Browser Performance metering with PhantomJS"].
...
// <vik>
Let's briefly review QUnit API components. 
You can find a typical QUnit in <<QUNIT_TEST,following listing>>.
...
<5> Typically, you need to use assertions to check to make sure code under the test gives expected result. Function http://api.qunitjs.com/ok/[`ok`] will examine first argument to by `true`. 

<6> A pair of functions `equal` and `notEqual` will check equivalence of the first and second arguments. Arguments could be an expressions as well.

<7> A code of the test is wrapped in immediate-invoked function expression (IIFE) and passes `jQuery` object as `$` variable.

More details about QUnit API you can find in http://api.qunitjs.com/[official documentation] and http://qunitjs.com/cookbook/[QUnit Cookbook].         
...
//<vik>
Consider the following example of login functionality. A `showAuthorizedSection` function will be invoked within `login` function after user entered correct username and password. We need to test that it's true.
...
<3> Create skeleton application in namespace of production code to provide the execution environment for production code.
...
----
<1> Wrap the Jasmine suite into the Ext JS class

<2> The rest of the code is very similar to the Jasmine code sample shown earlier in this chapter.

//<vik>

After setup the testing harness for SaveSickChild application we will explain the testing strategy for ExtJS application. We will start with testing the models and controllers, after that we will test our views.

===== Testing The Models

_SaveSichChild.org_ home page displays information about the fundraising campaigns using a chart and a table views backed by collection of `Campaign` models. 
A `Campaign` model should have three properties: `title`, `description`, `location`. The `title` property of the model should have a default value - `Default Campaign Title`. The `location` property of the model is required field.
In a spirit of TDD, let's write the <<CAMPMOD_SPEC,specification>> what will meet described above requirements.
...
----
<1> By default, `Ext.data.Model` caches every model created by the application in a global in-memory array. We need to clean up ExtJS model cache after each test run.

<2> Let's instantiate `Campaign` model class to check that model class exists.

<3> Next we need to check if model has all required properties. 

<4> Property `title` has default value.
...
// <vik>
In this situation, static code analysis tools, like JSHint, could be very handy to help identify typos and enforce the best practices of JavaScript community.
