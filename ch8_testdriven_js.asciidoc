...
To automate testing in all Web browsers, you can use Testem Runner. In this section we will show how to install and configure `Testem` to run Jasmine tests.
...
Testem runner is driven by a configuration file. By default, doesn't expect to have any configuration file. It will just pick any of JavaScript in project directory. If testem can identify any tests among that +.js+ files it will run it. 
You can configure testem to specify what files should be included. For that purpose, 
testem expects existence of `testem.json` file in the project directory. Example `testem.json` shown in <<TESTEM_JSON,following listing>>. 
...
----
<1> A `framework` directive used to specify test framework. Testem support Qunit, Jasmine and many more frameworks. You can find full list of supported frameworks on testem https://github.com/airportyh/testem#features[github page].

<2>  A list of test and production code source files. 

===== Running Tests 

Testem supports two running mode: development mode () and continuous integration. 
In the development mode, testem will start a development server.

.Testem development mode.
image::fig_08_13.png[]

This server url should be opened in any browser that you want run test against.

image::fig_08_14.png[]

You can connect to testem server any browser from network. For example, on <<TESTEM_IE, following screenshot>>, I have Internet Explorer 10 running under my Windows 7 virtual machine connected to testem server. 

[[TESTEM_IE]]
.Using testem to test code on IE with conjunction with previously introduced grunt with PhantomJS
image::fig_08_15.png[]

As you have seen on <<TESTEM_IE, previous screenshot>>, we can use 
...
In the continuous integration mode testem will examine system for all of the available browsers in your system and will execute tests on it. You can get list of the browsers that testem can use to run testes with `testem launchers` command. <<TESTEM_L,Here is simple output>> of the command. 
...
Now we can run our test simultaneously on all browsers that we have installed in  our computer - Google Chrome, Safari, Firefox, Opera and PhantomJS - with one command:
