...
// <vik>
Testem supports two running mode: test-driven-development mode (_tdd-mode_)  and continuous integration (_ci-mode_). In the tdd-mode, testem will start a development server.

.Testem tdd-mode.
image::fig_08_13.png[]

In tdd-mode, Testem doesn't spawn any browser automatically. On the contrary, you needs to open this url in browser that you want run test against to connect it to testem server. From this point, testem will execute tests in all connected browsers.  On the next <<TESTEM_MULTI,screeshot>> you can see we added different browsers including mobile version of Safari (running on iOS simulator).

[[TESTEM_MULTI]]
.Testem is running the tests on the multiple browsers.
image::fig_08_14.png[]

Because testem server actual HTTP server, you can connect remote browser as well. For example, the <<TESTEM_IE, following screenshot>> shows Internet Explorer 10 running under Windows 7 virtual machine connected to the testem server.

[[TESTEM_IE]]
.Using testem to test code on remote IE 10
image::fig_08_16.png[]

Running the tests with testem runner can be combined with previously introduced tool. In the <<TESTEM_AND_GRUNT,next screenshot>> you can see that testem used to run the tests on the actual browsers and and grunt run the tests on headless PhantomJS.
...
// <vik>
Automating our unit tests reduces the number of bugs and decreases the amount of time we need to spend manually testing our code.  

// </vik>
If automatically launched unit, integration, functional, and load test scripts don't reveal any issues, you can rest assured that the latest code changes did not break the application logic, and that it performs according to SLA. 
