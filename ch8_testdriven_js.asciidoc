...
Similar to unit tests, with BDD you write specifications that are small and and test one this. Specifications should be sentences. For example, "Calculator add two positive numbers". It will help you easy identify failed test by simply reading this sentence in test results report. As you will see next in this chapter, BDD framework provide very nice way to group, execute, and report JavaScript unit tests. 
...
+SpecRunner.html+ file is very similar to qunit html runner. 
...
Now let's update our Grunfile to run same sample specs. For that we need to copy content of +src+ folder of your Jasmine distribution into `app/js` folder of our project and content of `spec` folder into `test/spec` folder of our project. Also create folder `test/lib/jasmine` and copy there content of Jasmine distribution `lib` folder. <<FIG-3>>
