...
We gave you just enough theory to whet your appetite for implementing WebSocket in our Save The Child application. The goal is to create an auction so that people can bid and purchase various goods and have the proceeds go to Save The Child. Auctions require real-time communications: everyone interested in the particular auction item must be immediately notified of being overbid or of winnin. So we'll use WebSocket as a means for bidding and providing notifications of the changes in the auction.((("WebSocket protocol", "auction application using", id="ix_WSauct", range="startofrange")))
...
Since WebSocket is just a transport protocol, we need to come up with application-level protocol indicating how auction messages(((auction application, messaging protocol))) should be formatted in the client-server interaction. This is how we decided to do it:
