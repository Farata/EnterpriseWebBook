...
For more details, you can navigate Google Chrome to the secret URL chrome://net-internals, which provides a lot of useful information (see Figures pass:[<xref linkend="FIG9-6" xrefstyle="select: labelnumber"/>] and pass:[<xref linkend="FIG9-7" xrefstyle="select: labelnumber"/>]). You can find documentation about net-internals in http://www.chromium.org/developers/design-documents/network-stack/view-net-internals[Chromium Design Documents].
