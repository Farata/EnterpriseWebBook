...
In this chapter, we continue to use RequireJS (see <<tdd_with_javasctipt>>) as a framework for modularization. Using RequireJS, we're going to lazy-load some modules if and only if they are requested by the user.
