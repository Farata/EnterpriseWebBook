...
Before diving into the WebSocket protocol, let's become familiar with the standardized way of implementing  http://dev.w3.org/html5/eventsource/[Server-Sent Events]. The World Wide Web Consortium (W3C) has published an API for web browsers to allow them to subscribe to events sent by a server. All modern browsers support the `EventSource` object, which can handle events arriving in the form of DOM events. This is not a request-response paradigm, but rather a unidirectional data push, from server to browser. <<ex_subscribing_server_sent_events>> shows how a web browser can subscribe and listen to server-sent events.
