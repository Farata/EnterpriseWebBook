...
Every newly deployed Web application creates a new security hole in accessing of your organization's data. The hackers get access to the data by sneaking through the ports that are supposedly hidden behind the firewalls. There is no way to guarantee that your Web application is 100% secure. If it was never attacked by hackers most likelely it's too small and is of no interest to them. 
