...
This is a good example of limited access to a resource in a real life. The OAuth 2.0 authorization server gives the requesting application an _access token_ (think, valet key) so it can access, say, the charity application.
...
*******
Facebook is one of the authorization servers that offer an OAuth-based authentication and authorization API.  The online document https://developers.facebook.com/docs/javascript/quickstart/["Quickstart: Facebook SDK for JavaScript"] is a good starting point. 
