...
To have a better understanding of how JWTs are encoded, visit the http://openidtest.uninett.no/jwt[Federation Lab], which is a website with a set of tools for testing and verifying od various identity protocols. In particular, you can enter a JWT in clear text, select a secret signature, and encode the token by using the HS256 algorithm, as shown in <<FIG10-000>>.
