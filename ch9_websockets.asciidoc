...
<1> The constructor requires an endpoint URI and optional sub-protocols name string. The WebSocket client can explicitly request the server that application layer protocol need to be used for conversation. It could be a fully custom protocol or one of standard application layer protocols. In this case, it's possible to transfer existing standard protocols messages (such as SOAP, XMMP and etc) over the WebSocket connection.
