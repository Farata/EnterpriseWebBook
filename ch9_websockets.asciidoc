...
<1> The constructor requires an endpoint URI and optional sub-protocols name string.
<vik>The WebSocket client can explicitly request the server that application layer protocol need to be used for conversation. It could be fully custom protocol or one of standard application layer protocols. In this case, it's possible to transfer existing standard protocols messages (such as SOAP, XMMP and etc) over the WebSocket connection.
</vik>
...
The Portal[https://github.com/flowersinthesand/portal] is a server agnostic JavaScript library that not just provides a socket for browser-based applications that need two-way communication with servers, but also aims to utilize a full duplex connection for modern web application development. It supports various transports (long polling, HTTP streaming, WebSocket).

==== Atmosphere ====

https://github.com/Atmosphere/atmosphere[Atmosphere] is portable WebSocket/Comet framework supporting Java, Groovy, Scala. The Atmosphere Framework contains client and server side components for building Asynchronous Web Application. The majority of popular frameworks are either supporting [Atmosphere] or supported natively by the framework. The Atmosphere Framework supports all majors Browsers and Servers. Clent side component - Atmosphere.js[https://github.com/Atmosphere/atmosphere/wiki/jQuery.atmosphere.js-API] - simplified development of applications what require fall back from WebSocket protocol to long polling or HTTP streaming.

==== Native Applications ====

WebSockets can be used not only in a Web browsers communications, but in any applications that uses networking.
If you're developing native iOS or OSX applications you definitely want to check https://github.com/square/SocketRocket[SocketRocket] library developed by http://corner.squareup.com/[Square Engineering Team]. Square uses SocketRocket in their mobile payments application for iOS.
If you're developing native Android applications and want to use WebSocket protocol goodies in Android-powered devices check https://github.com/AsyncHttpClient/async-http-client[AsyncHttpClient] framework.
...
- The Protocol Buffers well documented.
...
- The Protobuf can be a bit smaller, especially when a lot of numeric values are transferred, but JSON the more common protocol in the JavaScript space and better supported and easier to integrate in application.
- This technology requires browser support for binary format. You can find current state of binary browser support http://caniuse.com/#search=binary
</vik>
...
<vik>
The problem with proxy servers for web applications that have a long-lived connection like  WebSocket is proxy servers may choose to close streaming or idle WebSocket connections, because they appear to be trying to connect with an unresponsive HTTP server. Additionally, proxy servers may also buffer unencrypted HTTP responses. It can introduce unpredictable latency during response streaming.
If you want to get more details how WebSockets-enabled application has to deal with proxies there is comprehensive research paper by Google's Peter Lubbers http://www.infoq.com/articles/Web-Sockets-Proxy-Servers/[WebSocket and proxy servers].
</vik>
...
Wireshark is a powerful and comprehensive monitoring tool for analyzing the network traffic.
<vik>
You can download version for you operation system from http://www.wireshark.org[official web site]. To start capturing WebSocket traffic we need to select network interface from list and click "Start" <<FIG9-15>>. In our case we're going to select +loopback+ interface to capture traffic on _localhost_.

[[FIG9-15]]
.Wireshark application main view
image::images/fig_09_15.png[]

The Wireshark going to capture all network activity. That is why is good idea to setup filter so we can see only required data. We want to capture HTTP traffic and TCP traffic on port +8080+ because our WebSocket server uses that port <<FIG9-16>>. Enter `http && (tcp.dstport==8080)` in filter text box and click apply.

[[FIG9-16]]
.Filter setup
image::images/fig_09_16.png[]

Now Wireshark is ready to sniff traffic of our application. You can start the auction session, place the bids. After you're done with the auction you can return to Wireshark window and analyze what we got. You can see initial handshake (+GET+ request <<FIG9-17>> and +Upgrade+ response <<FIG9-18>>)
...
After successful connection upgrade, Wireshark captured `http-alt` stream on 8080 port. This is our WebSocket connection. Right click on that row and select +Follow TCP Stream+ <<FIG9-19>>.

[[FIG9-19]]
.The +GET+ response with protocol upgrade
image::images/fig_09_19.png[]

On the next screen you can see details of WebSocket frame <<FIG9-20>>. We took this screenshot right after the auction application start. We can see here data with list of available auctions. The outgoing data marked with _red_ color and the incoming data marked with _blue_ color.

[[FIG9-20]]
.WebSocket frame
image::images/fig_09_20.png[]

We took next screenshot <<FIG9-21>> after the action has been finished. We can see here entire data what were sent over WebSocket connection.
