...
This chapter is about moving away from the HTTP protocol in Web applications. We'll start with a brief overview of the existing "legacy" Web networking, and then you'll learn how to use HTML5's Server-Sent Events (SSE) and WebSockets.

We'll implement fund-raising events and an interactive auction for our Save Sick Child application using WebSockets and  Server-Sent Events. The goal is to let you see the advantages of changing the way the Web client communicates with the server, and to prove that this is better than a regular HTTP we'll monitor the network with  Wireshark and Google Chrome Developer Tools.
...
<4> The property value contains name of sub-protocol <vik> used for conversation. This property populated by the browser after successful handshake with value from servers response parameter `Sec-WebSocket-Protocol` as described in <1>. </vik>

<5> WebSocket object can send text or binary data to server using on of the `send()` methods.
...
Once you start working with vanilla WebSocket API you may need to do some additional coding on you own. For example, if the client's browser doesn't support WebSocket natively, you need to make sure your code falls back to legacy protocols. The good news is that there are frameworks that can help you with this task. Such frameworks save development time, allowing you to do more with less code. We've included brief reviews of two frameworks that can streamline your Web application development with WebSockets.
<vik>
Mentioned below in next section frameworks try to solve common problem - leverage best available transport in browser or server and abstract developer form internals of used mechanism. Developer needs concentrate on application logic, program application against framework API and to "enjoy the ride". The rest hard work will be done by framework.
</vik>

==== The Portal ====

<vik>We're going to start our frameworks review with library called +Portal+. </vik>
The https://github.com/flowersinthesand/portal[Portal] is a server agnostic JavaScript library. It aims to utilize a WebSocket for modern web application development and provides unified API for various transports (long polling, HTTP streaming, WebSocket). <vik> Currently, once you decided to use WebSocket API for you next project, you need to remember about those unlucky users what still use, for example, Internet Explorer 8 and they don't have native WebSocket support there. At this case, your application should gracefully degrade and fall back to best available in browser communication technology. Writing code by hand to support all possible variants browsers and browser-specific technologies requires lot of time of testing and providing the compatibility fixes for different platforms. Let's see how +Portal+ library could help us.</vik>
...
<1> The Portal supports different transports and can fall back from WebSocket connection to streaming or long polling. Server have to support fall back as well but from client-side API there is no additional code required.

<2> Connecting to the WebSocket endpoint.

<3> The Portal API is event-based. It's very similar to W3C WebSocket API.

<vik>As you can see from previous snippet, API is very similar to W3C WebSocket API we have seen in this chapter. +Portal+ unifies the client-side. With defining transports array, we don't have to worry about how to handle message what sent by server with different transport.
The +Portal+ doesn't depend on any JavaScript libraries which and used by the next contestant framework - Atmosphere.</vik>

==== Atmosphere ====

<vik>Once you developed product - boxed application - and delivering it as web archive to your customers, you may want to support wide range of runtimes. Yes, your customers have WebSphere servers, JBoss server, WebLogic servers. Sad but true, "Write once - run everywhere" doesn't work as advertised. Currently, there are bunch of different implementation of server-side WebSocket framework. And each of them uses their own proprietary APIs. The upcoming Java EE 7 specification intends change situation for better or for worse. But there is framework what allows you to write portable web applications today.
</vik>
...
The client side component - https://github.com/Atmosphere/atmosphere/wiki/jQuery.atmosphere.js-API[Atmosphere.js] - simplifies the development of Web applications that require a fall back from WebSocket protocol to long polling or HTTP streaming. The Atmosphere Framework hides the complexity of the asynchronous APIs, which differ from server to server and make your application portable among them. Atmosphere provides compatibility layer for all major Java application servers that allow to select best available on server transport.

<vik>
As was mentioned earlier, the Atmosphere framework supports wide range of Java based server-side technologies via set of https://github.com/Atmosphere/atmosphere/wiki/Atmosphere-PlugIns-and-Extensions[extensions and plugins]. Even though, Atmosphere https://github.com/Atmosphere/atmosphere-jsr356[supports] upcoming Java API for WebSocket. So you can have best of 2 worlds - standard API and application portability.
</vik>
...
<vik>
For example, let's create protocol buffer +proto+ file for our "Save Sick Child" what will represent a donation information.
...
<1> The protobuf supports packages. You can organize you protobuf messages to prevent naming conflicts among different projects.

<2> As you can see, here we're using Java specific protobuf option to define in what package generated code will reside.

<3> Start define our custom message with `message` keyword.

<4> Each message field can be `required`, `optional` or `repeated`. The `required` and `optional` modifier are self explanatory. During serialization / deserization process protobuf framework will check message for existence of fields and if required property will be missing will throw runtime exception. The `repeated` modifier is used to create dynamic sized arrays.

<5> The protobuf supports nested messages.

<6> There are many standard types available as field types:  +string+, +int32+, +float+, +double+, and +bool+. You can also define custom type and set it as field type.

After you created +donation.proto+ file, we can use `protoc` compiler to generate Java classes form it.
...
<2> All required code for serialization / deserilization of +Donation+ message will be included in +Donation.java+. We're not going to publish this code here but you can always generate this code by yourself from provided above message code.

You can check protobuf compiler for you language in http://code.google.com/p/protobuf/wiki/ThirdPartyAddOns[protobuf wiki page]. If you want to make yourself familiar with protobuf technology check https://developers.google.com/protocol-buffers/docs[documentation] and https://developers.google.com/protocol-buffers/docs/tutorials[tutorials].
Here some considerations about protobuf usage for your project.
</vik>
