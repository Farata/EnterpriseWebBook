...
<vik - this is quote. I want it untouched>
...
We're going to implement monitoring of the fund-raising events and an interactive auction for our Save Sick Child application using WebSockets and  Server-Sent Events. We'll use Wireshark, a network monitoring tool, and <vik>Google Chrome Developer Tools </vik> to see benefits of using WebSockets protocol.

All server-side functionality supporting this chapter is written in Java, using <vik>Java API for WebSocket http://java.net/projects/tyrus[reference implementation] </vik> what will be part of upcoming Java EE 7 specification running on http://dlc.sun.com.edgesuite.net/glassfish/4.0/promoted/[latest promoted builds Glassfish Application Server]. If you don't know Java, just treat this server side setup as a service that supports WebSockets. For Java developers interested in diving into the server-side, we'll provide the source code and brief comments as a part of the code samples that come with this book.
<vik>
We're going to show the server-side data push with Server-Sent Events and WebSockets and compare them.
Also we will do brief overview of popular existing frameworks what can streamline you WebSockets application development</vik>
...
<vik>
WebSockets is a bi-directional full-duplex socket-based protocol. According to http://tools.ietf.org/html/rfc6455[RFC 6455]  - Internet Engineering Task Force (IETF) standard document - the goal of WebSocket technology is to provide mechanism for web applications what need two-way communication with servers. This technology shouldn't rely on «HTTP-hacks» or on opening multiple connections using +XMLHttpRequest+ or `<iframe>` and long polling.
The idea behind WebSockets is straightforward:
...
Also WebSocket protocol defines the new URI's:

- `ws` what identifies a WebSocket server and resource name.
- `wss` a WebSocket server and resource name and indicates that traffic over that connection is to be protected via TLS (including standard benefits of TLS such as data confidentiality and integrity and endpoint authentication).
...
----------------------------------------------------------------------
<vik>
<1> The constructor requires an endpoint URI and optional sub-protocols name. Because, the WebSocket protocol defines how data will be transmitted over the wire and fragmentation of data it can be used to transport existing application layer protocols like SOAP, XMPP and etc.
<2> The WebSocket can be in four states.
<3> There are callback function of WebSocket object what will be invoked by browsers engine once network related events would be fired
<4> Using `send()` methods WebSocket object can send data to server. Data can be both text and binary.
</vik>
===== Client-Side API =====
...
<vik>The WebSockets protocol consists of an opening handshake</vik>. Handshake upgrades the connection from HTTP to the WebSockets protocol. It's an upgrade to a message based communication. We will discuss messages (a.k.a. frames) later in this chapter. Why upgrading from HTTP instead of starting with the TCP as a protocol in the first place? The reason is that the WebSockets operates on the same ports (80 and 443) as HTTP and HTTPS do. It's an advantage because all ports under 1024 are _privileged_, and they are handled differently than non privileged ones.
...
<vik>Shaving off hundreds of bytes of HTTP overhead and reducing the latency makes WebSocket worthwhile considering for following applications:</vik>
...
- The XML validation on the client is a complex task. <vik> As for now, there is no platform independent solution or API to perform validation programmatically based on XSD or DTD.

Great guide to the full spectrum of XML features and tools you can find in _XML in a Nutshell, 3rd Edition_ by Elliotte Rusty Harold and W. Scott Means.
</vik>
...
<vik>
Developer needs to specify how the serializable information to be structured by defining protocol buffer message types in +.proto+ files. Each protocol buffer message is a small logical record of information, containing a series of name-value pairs. This protocol buffer message file is language agnostic. The `protoc` utility compiles +proto+ files and produces language specific artifacts, e.g. +.java+, +.js+, etc files.
...
<vik>The WebSocket protocol itself is unaware of intermediary such proxy servers, firewalls, content filters. The proxy servers are commonly used for content caching, security and enterprise content filtering. </vik>
...
<2> Checking WebSocket object state. If WebSocket is open (`readyState===1`) we can send message. If not we just simply logging this in console. In real world situation if your users work on unstable networks such as cellular or 3G <vik> you definitely don't want to lose any bits of your data. It's good idea use local storage API to persist the data locally until application gets back online and resubmit the data. We're going to demonstrate this technique in further chapters when we're going to talk about mobile and hybrid applications.</vik>
...
<vik>
Now you can see, Developer Tools show length of payload data. On other hand, `chrome://net-internals` applet shows size of actual WebSocket frame. As we learned earlier in this chapter, overall size of frame slightly different from size of the payload. There are few bytes for frame header. Moreover, all outgoing messages should be masked and mask should be transfered to server (additional 32 bits = 4 bytes to frame size).
...
<vik>
Wireshark is powerful and comprehensive tool for analyzing network traffic.
</vik>
...
<1> In case of one auction participant is placed bid bigger that other rest of participants will receive update ...
<2> with updated best bid value.
+
...
After auction timeout, server broadcasts the message with auction results. In case of connected client is won the item message will contain congratulations. On the contrary, the client will get message that she isn't the winner. </vik>

=== Summary

This chapter was covering new the WebSocket protocol. A smaller portion of this chapter was legacy options for real-time communication for web applications. We showed the benefits of using WebSocket protocol in applications and how can improve application performance by reducing latency and HTTP-headers overhead. We learned how to integrate WebSocket-based functionality to existing HTTP-based application. We learned how to use network monitoring capabilities of Google Chrome Developer Tools and Wireshark application for sniffing WebSocket traffic.
