...
Also WebSocket protocol defines two new URI schemes `ws` and `wss` for unencrypted and encrypted connections respectively. The `ws` URI schema identifies that a WebSocket connection will be established using TCP/IP protocol without encryption. The`wss` URI schema identifies that traffic over that connection will be protected via Transport Layer Security (TLS) including standard benefits of TLS such as data confidentiality and integrity and endpoint authentication. Apart from the scheme name, the rest of the URI components are defined to use URI generic syntax. You will get a better understanding of each of these statements after reading the section WebSocket API section later in this chapter.
...
Essentially, the WebSocket protocol introduces only the new type of transport and doesn't define a new application layer protocol. In this case, it's possible to transfer existing standard protocols messages (such as SOAP, XMMP and etc) on top of WebSocket. With filling this array with protocol names, we tell the server that our client can use standard protocols.
...
The Wireshark going to capture all network activity. That is why is good idea to setup filter so we can see only required data. We want to capture HTTP traffic and TCP traffic on port +8080+ because our WebSocket server uses that port <<FIG9-16>>. Enter `http && (tcp.port==8080)` in filter text box and click apply.
