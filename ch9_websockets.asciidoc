...
This chapter is about moving away from the HTTP protocol in Web applications. That's right, we'll start with introducing existing "legacy" options for creating interactive Web applications, and then you'll learn how to use HTML5's Server-Sent Events (SSE) and WebSockets.

We'll implement fund-raising events and an interactive auction for our Save Sick Child application using WebSockets and  Server-Sent Events. The goal is to let you see the advantages of changing the way the Web client communicates with the server, and to prove that this is better than a regular HTTP we'll monitod the network with  Wireshark and Google Chrome Developer Tools.
...
The W3C expert group uses http://en.wikipedia.org/wiki/Interface_description_language[Interface Description Language] to describe what the WebSocket interface should look like. This is how it looks:
...
<1> The constructor requires an endpoint URI and optional sub-protocols name string. <vik>A sub-protocol is application-level protocol layered over the WebSocket Protocol. The client-side application can explicitly indicate what sub-protocols are acceptable for conversation. That string will be send to server with initial handshake as `Sec-WebSocket-Protocol` +GET+ request header field. If the server supports one of requested protocols it selects at most one of acceptable protocols and echoes that value in the same header parameter `Sec-WebSocket-Protocol` in handshake response. Thereby the server indicates that it has selected that protocol. It could be a fully custom protocol or one of standard application level protocols. It's possible to transfer existing standard protocols messages (such as SOAP, XMPP and etc) over the WebSocket connection. We'll discuss handshake later in this chapter in <<HANDSHAKE>> section</vik>
...
<4> <vik>The property value contains name of used sub-protocol.</vik>
...
Once you start working with vanilla WebSocket API you need implement bunch of things by you own. E.g. in case client's browser doesn't support WebSocket natively, you need to fall back to legacy protocols. In this case, it's good idea to look around and find existing frameworks that can help you. Frameworks always can save you time in development, allowing you to do more with less code. Here is couple frameworks that can save you time and streamline your WebSocket application development.

==== Atmosphere ====
<vik>
https://github.com/Atmosphere/atmosphere[Atmosphere] is a *portable* WebSocket framework supporting Java, Groovy, Scala. The Atmosphere Framework contains both client and server components for building Asynchronous Web Applications. Atmosphere transparently supports WebSockets, Server Side Events, Long-Polling, HTTP Streaming and JSONP. The client side component - https://github.com/Atmosphere/atmosphere/wiki/jQuery.atmosphere.js-API[Atmosphere.js] - simplifies the development of Web applications that require a fall back from WebSocket protocol to long polling or HTTP streaming. The Atmosphere Framework hides the complexity of the asynchronous APIs, which differ from server to server and make your application portable among them. Atmosphere provides compatibility layer for all major Java application servers that allow to select best available on server transport.

==== The Portal ====

The https://github.com/flowersinthesand/portal[Portal] is a server agnostic JavaScript library. It aims to utilize a WebSocket for modern web application development and provides unified API for various transports (long polling, HTTP streaming, WebSocket).
...
<3> The API event-based and very similar to W3C WebSocket API.

</vik>

==== Native Applications ====

WebSockets can be used not only in for the Web, but in any applications that use networking. If you're developing native iOS or OSX applications check https://github.com/square/SocketRocket[the SocketRocket] library developed by http://corner.squareup.com/[Square Engineering Team]. Square uses SocketRocket in their mobile payments application for iOS. If you're developing native Android applications and want to use WebSocket protocol goodies in Android-powered devices check https://github.com/AsyncHttpClient/async-http-client[the AsyncHttpClient] framework.
...
- A message is encoded to the compact and optimized protocol buffer binary format. Details of encoding format can be found in https://developers.google.com/protocol-buffers/docs/encoding[Protocol Buffers documentation website]
- Wide range of programming languages are supported by Google (Java, C++, Python) and by http://code.google.com/p/protobuf/wiki/ThirdPartyAddOns[community].
- The Protocol Buffers are well documented.

Cons:

- The binary format is not human readable.
- The Protobuf can be a bit smaller, especially when a lot of numeric values are transferred https://developers.google.com/protocol-buffers/docs/encoding[by encoding algorithm], but JSON is natively supported by the JavaScript and doesn't require any additional parser implementation.
- Protobuf requires the browser to support the binary format, but not all of the do it just yet. You can find which browser support raw binary data at http://caniuse.com/#search=binary.
