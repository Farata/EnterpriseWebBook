...
TIP: The author of this book use [http://nginx.com/]NGINX, a hugely popular load balancer and proxy server to serve static resources (e.g. images and text files), balance the load between Java servers, and for SSL offloading (turning Web browser's HTTPS requests into HTTP). Recently NGINX started supporting WebSockets protocol.
