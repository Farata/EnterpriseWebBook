...
protocol. It could be a fully custom protocol or one of standard application level protocols. It's possible to transfer existing standard protocols messages (such as SOAP, XMPP and etc) over the WebSocket connection. We'll discuss handshake later in this chapter in <<HANDSHAKE>> section</vik>
...
- The Protobuf uses compact binary format.
- Google ships utilities for Java, Python and C++. Community contributes to the project with tooling for other languages.
- The Protocol Buffers are well documented.

Cons:

- The binary format is not human readable.
- The Protobuf can be a bit smaller, especially when a lot of numeric values are transferred, but JSON is more widely supported in the JavaScript space and is easier to integrate in the application.
