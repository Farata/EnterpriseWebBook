...
Also WebSocket protocol defines two new URI's:

- `ws` what identifies a WebSocket server and resource name.
- `wss` a WebSocket server and resource name and indicates that traffic over that connection is to be protected via Transport Layer Security (TLS)including standard benefits of TLS such as data confidentiality and integrity and endpoint authentication.

We will explain each of these statements later in this chapter while introducing WebSocket API.
...
<1> The constructor requires an endpoint URI and optional sub-protocols name. Because, the WebSocket protocol defines how the data will be transmitted over the wire and fragmentation of data it can be used to transport existing application layer protocols like SOAP, XMPP and etc.
