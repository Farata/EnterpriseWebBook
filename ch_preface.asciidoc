...
The source code for all versions of the Save The Child application are available for download from O'Reilly at the book's http://bit.ly/efficient-web-development[catalog page]. There is also a https://github.com/Farata/EnterpriseWebBook_sources[GitHub repository] where the authors keep the source code of the book examples. 
